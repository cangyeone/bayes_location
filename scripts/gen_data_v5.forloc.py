...
class Config:
    ...
    events_per_xy_range: Tuple[int, int] = (5, 8)

    # station distribution (INSIDE inset region)
    station_z_km: float = 0.0
    station_inset_km: float = 400   # x in [xmin+inset, xmax-inset], y in [ymin+inset, ymax-inset]
    ...
    # output
    out_arrivals: str = "run_fm3d/data/loc.synth_arrivals_skfmm_noise.new.txt"
    out_stations: str = "run_fm3d/data/loc.synth_stations.new.txt"
    out_events_vel: str = "run_fm3d/data/loc.synth_events_vel.new.txt"
    out_stations_vel: str = "run_fm3d/data/loc.synth_stations_vel.new.txt"
...
def solve_source_group(
    ...
    for ev in ev_list:
        ...
        for i_pick, idx in enumerate(p_sel):
            ...
            lines.append(
                f"   SC {sta_code:<10s}  P     {at_abs:10.4f}   {t_obs:8.4f} "
                f"0.00e+00   {vp_sta:.3f}/{vs_sta:.3f}    0.300   {float(dist_all[idx]):7.4f}   "
                f"{lon_s:9.4f}   {lat_s:8.4f}"
            )
        ...
        for j_pick, idx in enumerate(s_sel):
            ...
            lines.append(
                f"   SC {sta_code:<10s}  S     {at_abs:10.4f}   {t_obs:8.4f} "
                f"0.00e+00   {vp_sta:.3f}/{vs_sta:.3f}   0.400   {float(dist_all[idx]):7.4f}   "
                f"{lon_s:9.4f}   {lat_s:8.4f}"
            )
