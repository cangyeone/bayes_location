...
ckpt_path = 'ckpt/time.v3.1.eikonal.pt'
...
EVT  = lc.IO_JSON("run_fm3d/data/svi/event_picks.json",rw_type='r')
# The event observations are split in a dictionary format where each key is given by the event ID
event_ids = EVT.keys()
new_evt = {}
count = 0 
for eid, val in EVT.items():
    # Each val is a dictionary with keys including 'picks' representing a pandas dataframe
...
Stations       = pd.read_csv("run_fm3d/data/svi/stations.csv",names=['Network','Station','Y','X','Elevation'],sep=r'\s+')
...
LocMethod.LocateEvents(new_evt,Stations,output_plots=False, output_path="run_fm3d/data/svi/out", timer=True)
t2 = time.perf_counter() 
print('Total location time for {} events: {:.2f} seconds'.format(len(event_ids),t2-t1))