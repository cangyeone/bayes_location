...
def idx_takeoff_angle_from_tt(model, x, xs, idx=0, degrees=True, z_is_depth_positive=True):
    """
    model: PINNTravelTime, output (B,2) where [:,0]=Tp, [:,1]=Ts
    x  : (B,3) receiver [xr,yr,zr] in km
    xs : (B,3) source   [xe,ye,ze] in km
...
class PINNTravelTime(nn.Module):
    ...
    def forward(self, x, xs):
        ...
        out = out * 10.0
        return out
class PINNTravelTimePnSn(nn.Module):
    def __init__(self, hidden_dim=256):
        super().__init__()
        self.hidden_dim = hidden_dim
        self.net_merge = nn.Sequential(
...
travel_time_pnsn = PINNTravelTimePnSn()
travel_time_pnsn.eval()
travel_time_pnsn.to(device) 
yy1 = []
...
for i in range(500):
    xx.append(i)
    xr = torch.tensor([i,    0, 0.0], device=device)   # km
    xs = torch.tensor([0, 0, 30.0],  device=device)    # km (深度8km)
...
yy3 = []
ckpt_path = 'ckpt/time.real.pnsn.v1.0.pt'
travel_time_pnsn.load_state_dict(torch.load(ckpt_path, map_location='cpu')["model_state"])
travel_time_pnsn.to(dtype)
# 单个事件-台站
xx = []
...
for i in range(500):
    ...
    xr = torch.tensor([i,    0, 0.0], device=device)   # km
    xs = torch.tensor([0, 0, 30.0],  device=device)    # km (深度8km)
    angle_deg, dir_unit, grad = idx_takeoff_angle_from_tt(travel_time_pnsn, xr, xs, idx=2, degrees=True)
    yy3.append(angle_deg.item())


plt.plot(xx, yy1, c="r", label="Pg takeoff angle\n from synth data v1.0")
plt.plot(xx, yy2, c="b", label="Pg takeoff angle\n from real data v1.0")
plt.plot(xx[100:], yy3[100:], c="g", label="Pn takeoff angle\n from real data v1.0")
