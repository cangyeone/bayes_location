...
def read_catalog1(path):
    """
    Read our located catalog JSON (new format only):
      {
        "<id>": {
          "Picks": ...,
          "location": {
            "Hypocentre": [lon, lat, dep_km],
            ...
          }
        },
        ...
    ...
    Return DataFrame with columns: id, lat1, lon1, dep1
    """
    path = Path(path)
    ...
    for k, v in data.items():
        ...
        loc = v.get("location", None)
        print(loc)
        if not isinstance(loc, dict):
            n_missing += 1
            continue

        hypo = loc.get("Hypocentre", None)
        if not (isinstance(hypo, (list, tuple)) and len(hypo) >= 3):
            n_missing += 1
            continue
        ...
        records.append(dict(id=evid, lat1=lat, lon1=lon, dep1=dep))

    df = pd.DataFrame(records)
    if df.empty:
        raise RuntimeError(
            f"No events parsed from JSON: {path}\n"
            f"total={n_total}, bad_id={n_bad_id}, missing_fields={n_missing}, bad_hypo={n_bad_hypo}\n"
            f"Hint: ensure each item has v['location']['Hypocentre'] as [lon,lat,dep_km]."
        )
...
# =========================
def main():
    cat1 = read_catalog1("run_fm3d/data/svi/out/Catalogue.json")
