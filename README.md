...
| SWChinaCVM velocity profiles → projected velocity grid | `python -m bayesloc build-grid` |
...
### 4.3 Generate the fictitious demo grid and train

`work/demo/velocity.npz` is generated by [examples/make_demo.py](examples/make_demo.py), not downloaded. The script writes the axes and Vp/Vs arrays together with `geometry.json`. It uses 5 km grid spacing, `Vp = 5.8 + 0.02*z + 0.1*sin(x/20)*cos(y/20)` in km/s, and `Vs = Vp/1.73`. This is an artificial demonstration field, separate from the script's homogeneous analytic arrival CSVs.

Create that file before passing it to `--grid`:
...
### 4.4 Use SWChinaCVM-V2.0 as the velocity model

For a public regional model, use [SWChinaCVM-V2.0](https://github.com/liuyingustc/SWChinaCVM-V2.0). The [complete worked example](docs/SWCHINA_CVM.md) covers the pinned download, depth datum, interpolation, grid construction, FMM labels, training, and subsequent location inputs.

Download the **mean-sea-level** file `SWChinaCVMv2.0.txt.wrst.sea_level` using that guide, then run:
...
python -m bayesloc generate-fmm \
  ...
  --n-sources 200 --receivers-per-source 32 \
  --output work/sw-china-cvm-v2/train.fmm.npz
...
`build-grid` automatically derives the projection center from the model's bounding box, writes a `(9,41,41)` velocity grid, and saves matching geometry and provenance. Regional coordinates remain in ignored local files. The example uses a local 200 km × 200 km box and sea-level receivers; it is not full-model coverage or a real station network. Use `--geometry` for a verified local projection/domain, and regenerate labels and retrain to cover station elevations.

The converter interpolates velocity in projected x/y and depth, rejects extrapolation, and retains projection/datum metadata so FMM can reject mismatched geometry. This grid is distinct from the fictitious demo; do not combine its velocity data with `examples/geometry.json`. The surface-relative `SWChinaCVMv2.0.txt` cannot be substituted without a proper datum conversion. See [method and limits](docs/SWCHINA_CVM.md#6-interpolation-method-and-limits).
...
| [bayesloc/velocity.py](bayesloc/velocity.py) | Public velocity-profile conversion, projection, and interpolation |
...
| [tests/test_velocity.py](tests/test_velocity.py) | Fictitious velocity-profile interpolation and FMM handoff tests |
