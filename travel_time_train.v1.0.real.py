...
def read_china_loc(sta_loc_path: str) -> Dict[str, Tuple[float, float, float]]:
    """
    Parse lines like:
      ZZ DEMO 00 0.1000 -0.1000 0 ZZ.DEMO.00 # fictitious station
    ...
    with open(pha_path, "r", encoding="utf-8", errors="ignore") as f:
        for line in f:
            ...
            if s.startswith("#EVENT"):
                ...
                parts = s.split()
                # Example:
                #EVENT DEMO.200001010000.0001 eq 2000 01 01 001 00 00 00 000000 LOC 0.000 0.000 DEP 10 MAG ML 1.0
                if len(parts) < 18:
                    cur = None
                    continue
                evid = parts[1]
...
def events_to_samples(
    ...
    """
    Outputs:
      Xr (N,3) station xyz in km  [xr, yr, zr]
      Xs (N,3) event   xyz in km  [xe, ye, ze]
      Y  (N,2) travel times in sec: [tp, ts], missing = -1

    Rules:
      - project lon/lat to AEQD; convert meters -> km
      - keep only if BOTH station and event in window |x|,|y|<=1000 km
      - for each event-station:
          P: prefer Pn, else Pg
          S: prefer Sn, else Sg
        if multiple picks exist for same phase family, take minimum rel_t
    """
    meta = json.loads(Path(meta_json).read_text(encoding="utf-8"))
    lon0 = float(meta["lon0"])
    lat0 = float(meta["lat0"])
    ...
    for ev in events:
        ...
        # aggregate picks per station with family preference
        by_sta: Dict[str, Dict[str, float]] = {}  # keys: 'Pn','Pg','Sn','Sg' -> min rel_t
        for pk in ev.picks:
            sta_key = f"{pk.net}.{pk.sta}"
            d = by_sta.get(sta_key, {})
        ...
        for sta_key, d in by_sta.items():
            lon_s, lat_s, elev_km = station_table[sta_key]
            #elev_km = float(elev_m) / 1000.0 
            sx_m, sy_m = fwd.transform(lon_s, lat_s)
            sx_km, sy_km = sx_m / 1000.0, sy_m / 1000.0
            if abs(sx_km) > xy_km_limit or abs(sy_km) > xy_km_limit:
                continue

            # station elevation is negative; you asked "台站高程为负"
            if use_station_elev_as_z:
                # force negative (in case the file has positive elevations)
                zr_km = -elev_km
            else:
                zr_km = 0.0

            # prefer Pn over Pg; Sn over Sg
            tp = d.get("Pn", d.get("Pg", -1.0))
            ts = d.get("Sn", d.get("Sg", -1.0))

            Xr_list.append([sx_km, sy_km, zr_km])
            Xs_list.append([ex_km, ey_km, ez_km])
            Y_list.append([float(tp), float(ts)])
    ...
    proj_meta = dict(
        ...
        phase_rule="P: Pn>Pg ; S: Sn>Sg ; rel_t=pick-origin ; rel_t<=50s",
    )
    return Xr, Xs, Y, proj_meta
...
def main():
    ...
    ap.add_argument("--rel_t_max", type=float, default=150.0)
    ...
    ap.add_argument("--save_ckpt", default="ckpt/time.real.v1.0.pt", help="checkpoint path to save (e.g., pinn_tt.ckpt.pt)")
    ...
    print(Xr[:10], Xs[:10], Y[:10])
    print(f"[OK] samples: {len(Y)}")
    print(f"[INFO] missing ratio: P={(Y[:,0] < 0).mean():.3f}, S={(Y[:,1] < 0).mean():.3f}")
