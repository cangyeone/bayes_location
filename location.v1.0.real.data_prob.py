...
def gibbs_mh_location_multi(
    ...
    *,
    ...
    return {
        ...
        'lambdaP_samples': lambdaP,
        'lambdaS_samples': lambdaS, 
...
def read_event_file(file_path, station_path):
    ...
    Output matches the original function:
    ...
    def parse_time(s: str) -> datetime.datetime:
        """
        Robust datetime parser for strings like:
        '2022-09-05 03:30:33.730000'
        '2022-09-05 07:00:19'
        '2022-09-05T07:00:19'
        '2022-09-05T07:00:19.123Z'
        '2022-09-05 07:00:19.123+08:00'
        Returns naive datetime (timezone info stripped if present).
        """
        s = (s or "").strip()
        if not s:
        ...
        # Remove timezone offset like +08:00 or -07:00 (keep local clock time)
        # If you actually need to convert to UTC, tell me and I'll implement proper conversion.
        ...
        # Last resort: sometimes fractional seconds have fewer than 6 digits: '.1', '.12', '.1234'
        m = re.match(r"^(\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2})(\.\d+)$", s)
        if m:
            base, frac = m.group(1), m.group(2)[1:]  # remove dot
            frac = (frac + "000000")[:6]            # right-pad / truncate to 6
    ...
    def parse_station_key(s: str) -> str:
        # 'SC.LDXX' -> 'SCLDXX'，兼容旧代码 station="".join(parts[:2])
        s = s.strip()
        if not s:
            return ""
    ...
    # 优先级：同一台同一类(P或S)出现多个震相时保留更“强”的那个
    # 你要求 Pn 优先于 Pg，这里做成通用规则
    PHASE_PRIORITY = {
        "Pn": 30, "Pg": 20, "P": 10,
        "Sn": 30, "Sg": 20, "S": 10,
    }
    ...
    while i < len(lines):
        ...
        # -------------------------
        # 1) parse event header line
        # -------------------------
        header_line = lines[i].lstrip("#").strip()
        cols = [c.strip() for c in header_line.split(",") if c.strip() != ""]

        # cols 示例：
        # [0]=LD.20220905033033730000
        # [1]=2022-09-05 03:30:33.730000
        # [2]=102.025 (lon)
        # [3]=29.581 (lat)
        # [4]=6 (depth)
        # 后面可能还有 mag 等字段，不保证固定

        evid_str = cols[0] if len(cols) > 0 else f"EV{i}"
        # 尽量提取里面最长的数字串做 evid（保持你后续流程仍然用 int）
        m = re.findall(r"\d+", evid_str)
        if m:
            evid = int(max(m, key=len))
        else:
            evid = i  # fallback
        ...
        # mag 字段不确定：尽量从后面抓两个，否则置 0
        mag1 = 0.0
        mag2 = 0.0
        if len(cols) >= 7:
            # header 中第 6/7 个位置在你的例子里是 3,3
            try:
                mag1 = float(cols[6])
            except:
                mag1 = 0.0
        ...
        # -------------------------
        # 2) parse phase lines until next header
        # -------------------------
        i += 1
        while i < len(lines) and (not is_header(lines[i])):
            parts = [c.strip() for c in lines[i].split(",")]
            # 你的例子字段：
            # 0=Pg, 1=Pg, 2=1.0, 3=1.8497, 4=pick_time, 5=stlon, 6=stlat, 7=dist_km, 8=weight, 9=SC.LDXX
            if len(parts) < 10:
                i += 1
                continue

            ph_main = parts[0]          # 第一列为主
            # parts[1] 可能是另一种标记，不用
            try:
            ...
            station_raw = parts[9]
            station = parse_station_key(station_raw)

            event_info["phases"].append({
                "station": station,
                "travel_time": travel_time,
                "weight": weight,
                "phase_raw": ph_main,             # Pg/Pn/Sg...
                "phase_group": phase_group(ph_main)  # P/S
            })
            i += 1
    ...
    # =========================================================
    # 3) build loc_events + all_events (same as your old outputs)
    #    + apply "Pn over Pg" (and Sn over Sg) per station
    # =========================================================
    loc_events = []
    all_events = []
    ...
    for e in events:
        ...
        rcv = np.zeros([len(set_stations), 3], dtype=np.float32)
        T_p = np.ones([len(set_stations)], dtype=np.float32) * MISSING_VAL
        T_s = np.ones([len(set_stations)], dtype=np.float32) * MISSING_VAL

        # --- 3.2 先填充接收机位置（同你原逻辑）
        for st in set_stations:
            if st not in stloc:
                continue
            loc = stloc[st]  # 你原代码里 loc[0],loc[1],loc[2] 的顺序保持不变
            x, y = proj_fwd.transform(loc[0], loc[1])
            ...
            rcv[st2id[st], :] = np.array([x, y, z], dtype=np.float32)

        # --- 3.3 对每个 station 的 P/S 震相做“保留最优”选择
        # best_pick[(station, 'P'/'S')] = (priority, travel_time)
        best_pick = {}
        for p in e["phases"]:
            st = p["station"]
            ...
            g = p["phase_group"]  # 'P' or 'S'
            if g not in ("P", "S"):
                continue

            pr = phase_priority(p["phase_raw"])  # Pn > Pg ...
            key = (st, g)
            if key not in best_pick or pr > best_pick[key][0]:
                best_pick[key] = (pr, p["travel_time"])

        # --- 3.4 写入 T_p / T_s
        for (st, g), (_, tt) in best_pick.items():
            idx = st2id[st]
            if g == "P":
                T_p[idx] = tt
            elif g == "S":
        ...
        locinfo = {
            ...
            "T_p": torch.tensor(T_p, device=device).float(),
            "T_s": torch.tensor(T_s, device=device).float(),
        }
...
def plot_dt_4events_jgr(
    events,
    ctlg,
    MISSING_VAL,
    event_ids,              # torch tensor, shape [Npick]
    sigmaP_samples,         # [Nsamp, Nevt]
    sigmaS_samples,         # [Nsamp, Nevt]
    xs_samples,             # [Nsamp, Nevt, 3]
    t0_samples,             # [Nsamp, Nevt]
    lambdaP_samples,        # torch tensor, expect [Npick]
    lambdaS_samples,        # torch tensor, expect [Npick]
    sidxs=(25, 539, 13, 20),
    out_png="figure/figs/dt_events/dt_4events.png",
    out_pdf="figure/figs/dt_events/dt_4events.pdf",
    zP=None, 
    zS=None, 
    pad_x_km=10.0,
    pad_t_s=2.0,
    use_t_quantile=False,
    qP=2.132,
    qS=2.132,
    add_legend=True,
):
    ...
    import torch
    sigmaP_samples = sigmaP_samples.float()
    sigmaS_samples = sigmaS_samples.float()
    xs_samples = xs_samples.float()
    t0_samples = t0_samples.float()
    lambdaP_samples = lambdaP_samples.float()
    lambdaS_samples = lambdaS_samples.float()
    event_ids = event_ids.long()

    # posterior mean (event-level)
    xs_mean_all = xs_samples.mean(dim=0)  # [Nevt,3]
    t0_mean_all = t0_samples.mean(dim=0)  # [Nevt]

    # --- 2x2 figure
    fig = plt.figure(figsize=(7.2, 5.6), dpi=300)
    gs = gridspec.GridSpec(2, 2, left=0.10, right=0.98, top=0.96, bottom=0.12,
    ...
    for k, e_idx in enumerate(sidxs):
        ax = axs[k]
        add_panel_label(ax, panel_labels[k])
        nice_axes(ax)

        ax.set_xlabel("Epicentral distance (km)")
        ax.set_ylabel("Travel time (s)")

        if e_idx >= len(events) or e_idx >= len(ctlg):
            ax.text(0.5, 0.5, "Index out of range", transform=ax.transAxes,
                    ha="center", va="center", fontsize=7, color="0.4")
            continue

        eve = events[e_idx]
        locinfo = eve["locinfo"]
        rcv = locinfo["rcv"].detach().cpu().numpy()  # [Ns,3]
        T_p = locinfo["T_p"].detach().cpu().numpy()  # [Ns]
        T_s = locinfo["T_s"].detach().cpu().numpy()  # [Ns]
        event_mask = (event_ids == e_idx) 
        zP_e_all = zP[event_mask] if zP is not None else None
        zS_e_all = zS[event_mask] if zS is not None else None
        print(zP_e_all.shape, T_p.shape)

        maskP = (T_p != MISSING_VAL)
        maskS = (T_s != MISSING_VAL)

        if not (maskP.any() or maskS.any()):
            ax.text(0.5, 0.5, "No valid picks", transform=ax.transAxes,
                    ha="center", va="center", fontsize=7, color="0.4")
            continue

        # mean source
        sx, sy, sz = xs_mean_all[e_idx].detach().cpu().numpy().tolist()
        t0_e = float(t0_mean_all[e_idx].item())

        # distances
        dist_all = np.sqrt((rcv[:, 0] - sx) ** 2 + (rcv[:, 1] - sy) ** 2)

        # observed travel times
        tP_obs_all = T_p - t0_e
        tS_obs_all = T_s - t0_e

        dist_P = dist_all[maskP]
        dist_S = dist_all[maskS]
        tP_obs = tP_obs_all[maskP]
        tS_obs = tS_obs_all[maskS]
        zPe = zP_e_all[maskP] if zP_e_all is not None else None
        zSe = zS_e_all[maskS] if zS_e_all is not None else None
        zPe = zPe.cpu().numpy()
        zSe = zSe.cpu().numpy()

        # ---- build sigP/sigS robustly (torch path)
        # event-level sigma mean
        sigmaP_e_mean = float(sigmaP_samples[:, e_idx].mean().item())
        sigmaS_e_mean = float(sigmaS_samples[:, e_idx].mean().item())

        # defaults: no errorbars
        yerrP = None
        yerrS = None

        try:
            # pick-level scale sigma/sqrt(lambda)
            # expect lambdaP_samples shape [Npick]
            sigP_all = sigmaP_e_mean / torch.sqrt(lambdaP_samples + 1e-12)
            sigS_all = sigmaS_e_mean / torch.sqrt(lambdaS_samples + 1e-12)

            # select this event picks (pick-level)
            sigP_e = sigP_all[event_ids == e_idx]
            sigS_e = sigS_all[event_ids == e_idx]

            # IMPORTANT: your original code assumes ordering matches station-order maskP/maskS.
            # We keep the same assumption; if mismatch, it will raise or size mismatch -> fallback.
            if sigP_e.numel() >= int(maskP.sum()):
                sigP_e = sigP_e[:int(maskP.sum())]
                yerrP = sigP_e.detach().cpu().numpy()
            if sigS_e.numel() >= int(maskS.sum()):
                sigS_e = sigS_e[:int(maskS.sum())]
                yerrS = sigS_e.detach().cpu().numpy()

            if use_t_quantile:
                if yerrP is not None: yerrP = qP * yerrP
                if yerrS is not None: yerrS = qS * yerrS

        except Exception as ex:
            # degrade gracefully: scatter only
            yerrP = None
            yerrS = None
            # 你想要看原因就打开下一行
            # print(f"[WARN] event {e_idx} yerr fallback due to: {ex}")

        # ---- plot P
        if dist_P.size > 0:
            if yerrP is None:
                hP = ax.plot(dist_P, tP_obs, "o", ms=2.2, color=colP, alpha=0.9, lw=0)[0]
            else:

                hP = ax.errorbar(dist_P[zPe>0.5], tP_obs[zPe>0.5], yerr=yerrP[zPe>0.5],
                                 fmt="o", markersize=2.2,
                                 color=colP, ecolor=eP,
                                 elinewidth=0.5, capsize=1.2,
                                 alpha=0.9)[0]
                hP1 = ax.scatter(dist_P[zPe<=0.5], tP_obs[zPe<=0.5], marker="$X_P$", s=30, color="0.6", alpha=0.9)
            if k == 0:
                legend_handles.append(hP); legend_labels.append("P")
                legend_handles.append(hP1); legend_labels.append("Outliers P")

        # ---- plot S
        if dist_S.size > 0:
            if yerrS is None:
                hS = ax.plot(dist_S, tS_obs, "^", ms=2.2, color=colS, alpha=0.9, lw=0)[0]
            else:
                hS = ax.errorbar(dist_S[zSe>0.5], tS_obs[zSe>0.5], yerr=yerrS[zSe>0.5],
                                 fmt="^", markersize=2.2,
                                 color=colS, ecolor=eS,
                                 elinewidth=0.5, capsize=1.2,
                                 alpha=0.9)[0]
                hS1 = ax.scatter(dist_S[zSe<=0.5], tS_obs[zSe<=0.5], marker="$X_S$", s=30, color="0.6", alpha=0.9)
            if k == 0:
                legend_handles.append(hS); legend_labels.append("S")
                legend_handles.append(hS1); legend_labels.append("Outliers S")
        # per-panel limits from plotted points + fixed pad
        d_all = []
        t_all = []
        if dist_P.size > 0: d_all.append(dist_P); t_all.append(tP_obs)
        if dist_S.size > 0: d_all.append(dist_S); t_all.append(tS_obs)
        if len(d_all) == 0:
            ax.text(0.5, 0.5, "No points after mask", transform=ax.transAxes,
                    ha="center", va="center", fontsize=7, color="0.4")
            continue

        d_all = np.concatenate(d_all)
        t_all = np.concatenate(t_all)
        ax.set_xlim(*_pad_lim(np.nanmin(d_all), np.nanmax(d_all), pad_x_km))
        ax.set_ylim(*_pad_lim(np.nanmin(t_all), np.nanmax(t_all), pad_t_s))

    if add_legend and len(legend_handles) > 0:
...
if __name__ == "__main__":
    ...
    sigmaP_samples = out["sigmaP_samples"]
    sigmaS_samples = out["sigmaS_samples"]
    lambdaP_samples = out["lambdaP_samples"]
    lambdaS_samples = out["lambdaS_samples"] 
    zP = out["zP_final"]
    zS = out["zS_final"]
    print(zP.shape)
    ...
    plot_dt_4events_jgr(
        ...
        lambdaP_samples=lambdaP_samples,
        lambdaS_samples=lambdaS_samples,
        zP=zP,
        zS=zS,
        sidxs=[25, 539, 13, 20],
        out_png="figures/figs/dt_events/dt_4events.v1.0.png",
        out_pdf="figures/figs/dt_events/dt_4events.v1.0.pdf",
        use_t_quantile=False,  # 你原逻辑：只画 sig
    )