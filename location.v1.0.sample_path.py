# mcmc_speedup_gmm_mask_nuts.py
# 功能汇总：
# 1) GMM 先验 (由地震目录 ctlg 拟合)，可与宽高斯混合
# 2) RW-MH 自适应步长 (Robbins–Monro 简化)
# 3) 混合核：GMM 独立提议 + RW 随机游走
# 4) NUTS 更新 xs（t0/σ² 仍为条件更新）
# 5) P/S 缺测支持：Tp/Ts 用 -12345 表示缺失，自动掩码

import math
import time
from typing import Optional, Dict, Tuple

import torch
import torch.nn as nn
from torch import Tensor
import matplotlib.pyplot as plt

import numpy as np 
import random 
# ================== 设备与随机种子 ==================

SEED   = 2024
torch.manual_seed(SEED); np.random.seed(SEED); random.seed(SEED)
# ================== 域与超参 ==================
# 建议数据侧把物理坐标无量纲化/归一到 [-1,1]^3
DOMAIN_MIN = -1000
DOMAIN_MAX =  1000 

# Godunov / 连续残差
USE_GODUNOV = True
H_REL       = 0.01    # 差分步长比例 h = H_REL * (DOMAIN_MAX - DOMAIN_MIN)

# 源点球 + 采样
EPS        = 0.02
B_MAX      = 16       # 每步用于 PDE/BC 的最大“唯一源点”数
M_PDE      = 1024     # 每源 PDE 采样点
K_BC       = 512      # 每源球面 BC 点
RAR_EVERY  = 500      # RAR 周期（步），0 关闭
RAR_TOPK   = 1024     # 每源 RAR 加密点数上限

# 课程式权重 & 损失
LPDE_START = 0.1
LPDE_END   = 3.0
LBC        = 10.0
LDAT_P     = 1.0
LDAT_S     = 1.0

# 优化
LR         = 1e-3
GRAD_CLIP  = 1.0
STEPS1     = 4000     # Adam
STEPS2     = 1000     # 第二阶段（Adam 或 LBFGS）
USE_LBFGS  = False

# 网络结构
HIDDEN     = 512 
LAYERS     = 8 
USE_FF     = True
FF_NUM     = 16
FF_B       = 8.0

# =============== 速度模型（默认常速；请替换为你的 3D 查询） ===============
VP0 = 6.0  # km/s
VS0 = 3.5

# ---------------------------
# 设备与全局设置
# ---------------------------
if torch.backends.mps.is_available():
    device = torch.device("mps")
elif torch.cuda.is_available():
    device = torch.device("cuda")
else:
    device = torch.device("cpu")

dtype = torch.float32  

# 缺测标记
MISSING_VAL = -12345.0




# -------------------------
# Model (as provided)
# -------------------------

class PINNTravelTime(nn.Module):
    def __init__(self, hidden_dim=256):
        super().__init__()
        self.hidden_dim = hidden_dim
        self.net_merge = nn.Sequential(
            nn.Linear(6, hidden_dim), nn.Tanh(),
            nn.Linear(hidden_dim, hidden_dim), nn.Tanh(),
            nn.Linear(hidden_dim, hidden_dim), nn.Tanh(),
            nn.Linear(hidden_dim, hidden_dim), nn.Tanh(),
            nn.Linear(hidden_dim, hidden_dim), nn.Tanh(),
            nn.Linear(hidden_dim, hidden_dim), nn.Tanh(),
            nn.Linear(hidden_dim, hidden_dim), nn.Tanh(),
            nn.Linear(hidden_dim, 2),
            nn.Softplus(), 
        )

    def forward(self, x, xs):
        # x : (B,3) receiver [xr,yr,zr]
        # xs: (B,3) source   [xe,ye,ze]
        inp = torch.cat([x, xs.expand_as(x)], dim=-1)  # (B,6)
        inp = inp / 1000.0
        out = self.net_merge(inp)
        out = out * 10.0 
        return out


model = PINNTravelTime().to(device).eval()
ckpt_path = 'ckpt/time.v1.0.pt'
model.load_state_dict(torch.load(ckpt_path, map_location='cpu')["model_state"])
model.to(dtype)

#ckpt_path = 'ckpt/travel_time.ps.v2.pth'
#model.load_state_dict(torch.load(ckpt_path, map_location='cpu'))
#model.to(dtype)


# ===========================
# 批处理：把 rcv(list) 打包成定长 padding
# ===========================
def pack_rcv_to_batch(rcv, *, dtype=torch.float32, device=None):
    """
    rcv: list[dict], 每个包含:
        'xr': [Ni,3], 'Tp':[Ni], 'Ts':[Ni]
    返回:
        XR: [NC, Nmax, 3]
        Tp, Ts: [NC, Nmax]
        exist_mask: [NC, Nmax]  (True=真实台站)
        maskP, maskS: [NC, Nmax]  (相位可用)
        NsP, NsS: [NC]  每事件有效样本数
    """
    assert isinstance(rcv, list) and len(rcv) > 0
    NC = len(rcv)
    Ni_list = [ev['xr'].shape[0] for ev in rcv]
    Nmax = max(Ni_list)

    def pad_2d(mat, fill=0.0):
        Ni = mat.shape[0]
        out = torch.full((Nmax, mat.shape[1]), fill, dtype=dtype, device=device)
        out[:Ni] = torch.as_tensor(mat, dtype=dtype, device=device)
        return out

    def pad_1d(vec, fill):
        Ni = vec.shape[0]
        out = torch.full((Nmax,), fill, dtype=dtype, device=device)
        out[:Ni] = torch.as_tensor(vec, dtype=dtype, device=device)
        return out

    XR = torch.stack([pad_2d(ev['xr'], 0.0) for ev in rcv], dim=0)           # [NC, Nmax, 3]
    Tp = torch.stack([pad_1d(ev['Tp'], MISSING_VAL) for ev in rcv], dim=0)   # [NC, Nmax]
    Ts = torch.stack([pad_1d(ev['Ts'], MISSING_VAL) for ev in rcv], dim=0)   # [NC, Nmax]

    exist_mask = torch.zeros((NC, Nmax), dtype=torch.bool, device=device)
    for i, Ni in enumerate(Ni_list):
        exist_mask[i, :Ni] = True

    maskP = (Tp != MISSING_VAL) & exist_mask
    maskS = (Ts != MISSING_VAL) & exist_mask

    NsP = maskP.sum(dim=1)  # [NC]
    NsS = maskS.sum(dim=1)  # [NC]
    return XR, Tp, Ts, exist_mask, maskP, maskS, NsP, NsS



# ===========================
# 批量对数似然（掩码聚合，逐事件）
# ===========================
@torch.no_grad()
def batched_log_lik(
    T_pred: torch.Tensor, Tp: torch.Tensor, Ts: torch.Tensor,
    t0: torch.Tensor, sigmaP2: torch.Tensor, sigmaS2: torch.Tensor,
    log2pi: torch.Tensor, maskP: torch.Tensor, maskS: torch.Tensor
) -> torch.Tensor:
    """
    T_pred: [NC, Nmax, 2], Tp/Ts: [NC, Nmax], 其余: [NC]
    return: per-event log-lik: [NC]
    """
    tp_model = T_pred[..., 0]
    ts_model = T_pred[..., 1]
    NC = Tp.shape[0]
    ll = Tp.new_zeros(NC)

    # P
    if maskP.any():
        resP = Tp[maskP] - t0.view(-1, 1).expand_as(Tp)[maskP] - tp_model[maskP]
        evt_id_P = torch.nonzero(maskP, as_tuple=False)[:, 0]
        contribP = -0.5 * (resP * resP) / sigmaP2[evt_id_P]
        ll.index_add_(0, evt_id_P, contribP)
        NP_evt = maskP.sum(dim=1)
        ll += -0.5 * (NP_evt * (log2pi + torch.log(sigmaP2)))

    # S
    if maskS.any():
        resS = Ts[maskS] - t0.view(-1, 1).expand_as(Ts)[maskS] - ts_model[maskS]
        evt_id_S = torch.nonzero(maskS, as_tuple=False)[:, 0]
        contribS = -0.5 * (resS * resS) / sigmaS2[evt_id_S]
        ll.index_add_(0, evt_id_S, contribS)
        NS_evt = maskS.sum(dim=1)
        ll += -0.5 * (NS_evt * (log2pi + torch.log(sigmaS2)))

    return ll

def build_phase_masks(Tp_obs: Tensor, Ts_obs: Tensor, missing_value: float = MISSING_VAL):
    """
    返回两种相位是否可用的布尔掩码（形状均为 (N,)）以及各自的样本数。
    """
    maskP = (Tp_obs != missing_value)
    maskS = (Ts_obs != missing_value)
    Np = int(maskP.sum().item())
    Ns = int(maskS.sum().item())
    return maskP, maskS, Np, Ns

def log_lik_from_pred_masked(
    T_pred: Tensor,
    Tp_obs: Tensor, Ts_obs: Tensor,
    t0_local: Tensor,
    sP2: Tensor, sS2: Tensor,
    log2pi: Tensor,
    maskP: Tensor, maskS: Tensor,
    Np: int, Ns: int,
) -> Tensor:
    """
    只在有观测的相位上计算似然。
    - 若某一相位完全缺失（N=0），跳过该相位的项。
    """
    tp_model = T_pred[:, 0]
    ts_model = T_pred[:, 1]
    ll = torch.tensor(0.0, dtype=Tp_obs.dtype, device=Tp_obs.device)

    if Np > 0:
        resP = Tp_obs[maskP] - t0_local - tp_model[maskP]
        llP = -0.5 * torch.sum(resP * resP) / sP2 - 0.5 * Np * (log2pi + torch.log(sP2))
        ll = ll + llP

    if Ns > 0:
        resS = Ts_obs[maskS] - t0_local - ts_model[maskS]
        llS = -0.5 * torch.sum(resS * resS) / sS2 - 0.5 * Ns * (log2pi + torch.log(sS2))
        ll = ll + llS

    return ll

def sample_invgamma(alpha: torch.Tensor, beta: torch.Tensor,
                    dtype: torch.dtype, device: torch.device) -> torch.Tensor:
    """
    采样 InvGamma(alpha, beta)；MPS 上回退 CPU。
    """
    alpha_cpu = torch.as_tensor(alpha, dtype=dtype, device='cpu')
    beta_cpu  = torch.as_tensor(beta,  dtype=dtype, device='cpu')
    g = torch.distributions.Gamma(concentration=alpha_cpu, rate=beta_cpu).sample(())
    invg = 1.0 / g
    return invg.to(device=device, dtype=dtype)
def sample_invgamma_batch(
    alpha: torch.Tensor,
    beta: torch.Tensor,
    eps: float = 1e-12,
) -> torch.Tensor:
    """
    批量采样 InvGamma(alpha_i, beta_i)，返回 shape=[NC]。
    - CUDA/CPU: 原地采样
    - MPS: 显式移到 CPU 做 Gamma 采样（MPS 缺少 gamma sampling kernel），再拷回

    约定：InvGamma(alpha, beta) 可由 G ~ Gamma(alpha, rate=beta)，取 1/G 得到。
    """
    assert alpha.device == beta.device, "alpha 和 beta 必须在同一 device 上"
    device = alpha.device
    dtype  = alpha.dtype

    # 数值/合法性保护
    alpha_ = alpha.to(dtype=dtype).clamp_min(eps)
    beta_  = beta.to(dtype=dtype).clamp_min(eps)

    if device.type == "mps":
        # --- MPS workaround: sample on CPU ---
        a_cpu = alpha_.detach().to("cpu")
        b_cpu = beta_.detach().to("cpu")
        g_cpu = torch.distributions.Gamma(concentration=a_cpu, rate=b_cpu).sample()
        g_cpu = g_cpu.clamp_min(eps)
        invg_cpu = (1.0 / g_cpu).to(dtype=dtype)
        return invg_cpu.to(device=device)
    else:
        g = torch.distributions.Gamma(concentration=alpha_, rate=beta_).sample()
        g = g.clamp_min(eps)
        return 1.0 / g

# ===========================
# GMM 拟合 + logpdf + 采样
# ===========================
@torch.no_grad()
def fit_gmm_em_torch(
    ctlg: Tensor,
    n_components: int = 5,
    n_iters: int = 50,
    reg_eps: float = 1e-3,
    device: Optional[torch.device] = None,
    dtype: torch.dtype = torch.float32,
    generator: Optional[torch.Generator] = None,
):
    """
    纯 Torch EM（3D 全协方差），返回 {w, mu, L, log_det}
    L 为 Cholesky 下三角，Σ = L L^T
    """
    assert ctlg.ndim == 2 and ctlg.shape[1] == 3
    device = device or ctlg.device
    dtype  = dtype
    X = ctlg.to(device=device, dtype=dtype)
    N, D = X.shape
    M = min(n_components, max(1, N))

    if generator is None:
        generator = torch.Generator(device=device)

    # KMeans++ 粗初始化
    idx0 = torch.randint(0, N, (1,), device=device)
    mu = [X[idx0]]
    for _ in range(1, M):
        dist2 = torch.stack([torch.sum((X - m)**2, dim=1) for m in mu], dim=1).min(dim=1).values
        probs = (dist2 + 1e-12) / (dist2.sum() + 1e-12)
        idx = torch.multinomial(probs, 1)
        mu.append(X[idx])
    mu = torch.cat(mu, dim=0)  # (M,3)

    Xc = X - X.mean(dim=0, keepdim=True)
    Sigma0 = (Xc.T @ Xc) / max(N - 1, 1)
    Sigma0 = Sigma0 + reg_eps * torch.eye(D, device=device, dtype=dtype)
    L0 = torch.linalg.cholesky(Sigma0)

    L = L0.expand(M, D, D).clone()
    w = torch.full((M,), 1.0 / M, device=device, dtype=dtype)

    def log_gauss_L(x, mu_k, L_k):
        y = x - mu_k  # (N,3)
        z = torch.triangular_solve(y.T, L_k, upper=False)[0].T
        quad = torch.sum(z * z, dim=1)
        log_det = 2.0 * torch.sum(torch.log(torch.diag(L_k)))
        return -0.5 * (quad + log_det + D * math.log(2.0 * math.pi))

    for _ in range(n_iters):
        # E-step
        log_resp = []
        for k in range(M):
            lg = log_gauss_L(X, mu[k], L[k]) + torch.log(w[k] + 1e-12)
            log_resp.append(lg)
        log_resp = torch.stack(log_resp, dim=1)  # (N,M)
        maxv, _ = torch.max(log_resp, dim=1, keepdim=True)
        resp = torch.exp(log_resp - maxv)
        resp = resp / (resp.sum(dim=1, keepdim=True) + 1e-12)

        Nk = resp.sum(dim=0) + 1e-12
        w = Nk / Nk.sum()

        # M-step
        mu = (resp.T @ X) / Nk.unsqueeze(1)
        for k in range(M):
            Xmk = X - mu[k]
            Sk = (resp[:, k].unsqueeze(1) * Xmk).T @ Xmk / Nk[k]
            Sk = Sk + reg_eps * torch.eye(D, device=device, dtype=dtype)
            L[k] = torch.linalg.cholesky(Sk)

    log_det = torch.zeros((M,), device=device, dtype=dtype)
    for k in range(M):
        log_det[k] = 2.0 * torch.sum(torch.log(torch.diag(L[k])))

    return {"w": w, "mu": mu, "L": L, "log_det": log_det}

def gmm_logpdf(x: Tensor, gmm: Dict[str, Tensor]) -> Tensor:
    """
    log sum_k w_k N(x|mu_k, Σ_k)；x:(...,3) -> (...,)
    """
    w, mu, L, log_det = gmm["w"], gmm["mu"], gmm["L"], gmm["log_det"]
    D = mu.shape[-1]
    x2d = x.view(-1, D)
    Ntot = x2d.shape[0]
    M = w.shape[0]
    logs = []
    for k in range(M):
        y = x2d - mu[k]
        z = torch.triangular_solve(y.T, L[k], upper=False)[0].T
        quad = torch.sum(z * z, dim=1)
        lg = -0.5 * (quad + log_det[k] + D * math.log(2.0 * math.pi)) + torch.log(w[k] + 1e-12)
        logs.append(lg)
    logs = torch.stack(logs, dim=1)  # (Ntot,M)
    m, _ = torch.max(logs, dim=1, keepdim=True)
    res = m.squeeze(1) + torch.log(torch.sum(torch.exp(logs - m), dim=1) + 1e-12)
    return res.view(x.shape[:-1])

def sample_gmm(gmm: Dict[str, Tensor], generator: Optional[torch.Generator] = None) -> Tensor:
    """
    从 GMM 采一个样本；返回 (3,)
    """
    w, mu, L = gmm["w"], gmm["mu"], gmm["L"]
    device = w.device
    dtype_ = w.dtype
    if generator is None:
        generator = torch.Generator(device=device)
    M = w.shape[0]
    comp = torch.multinomial(w, 1).item()
    z = torch.randn((3,1), device=device, dtype=dtype_)  # (3,1)
    x = (mu[comp].unsqueeze(1) + L[comp] @ z).squeeze(1)  # (3,)
    return x

def sample_gmm_batch(
    gmm: Dict[str, Tensor],
    NC: int,
    generator: Optional[torch.Generator] = None
) -> Tensor:
    """
    从 GMM 批量采样 NC 个样本；返回形状 (NC, 3)
    """
    w, mu, L = gmm["w"], gmm["mu"], gmm["L"]
    device = w.device
    dtype_ = w.dtype

    if generator is None:
        generator = torch.Generator(device=device)

    M = w.shape[0]

    # 1) 根据权重 w 为每个样本选择一个组件：shape = (NC,)
    comp = torch.multinomial(w, NC, replacement=True)

    # 2) 从标准正态生成 z：shape = (NC, 3, 1)
    z = torch.randn((NC, 3, 1), device=device, dtype=dtype_)

    # 3) 根据组件索引取 mu 和 L：
    # mu_selected: (NC, 3, 1)
    mu_selected = mu[comp].unsqueeze(2)

    # L_selected: (NC, 3, 3)
    L_selected = L[comp]

    # 4) 采样 x = mu + L @ z → shape (NC, 3)
    x = (mu_selected + L_selected @ z).squeeze(-1)

    return x


def make_log_prior_xs(
    *,
    gmm_prior: Optional[Dict[str, Tensor]] = None,
    wide_sigma: float = 1000.0,
    prior_mix: Optional[float] = None,
    device: torch.device,
    dtype: torch.dtype,
):
    """
    返回 log_prior_xs(x) 闭包；支持：
      - 仅 GMM ；仅宽高斯；或二者混合 log(a p1 + (1-a) p2)
    """
    wide_sigma2 = torch.as_tensor(wide_sigma**2, device=device, dtype=dtype)
    log_norm_wide = -0.5 * (3 * math.log(2.0 * math.pi) + 3 * torch.log(wide_sigma2))

    def log_wide(x: Tensor) -> Tensor:
        quad = torch.sum(x * x, dim=-1) / wide_sigma2
        return log_norm_wide - 0.5 * quad

    if gmm_prior is None and prior_mix is None:
        def _f(x: Tensor) -> Tensor:
            return log_wide(x)
        return _f

    if gmm_prior is not None and (prior_mix is None):
        def _f(x: Tensor) -> Tensor:
            return gmm_logpdf(x, gmm_prior)
        return _f

    if gmm_prior is None and (prior_mix is not None):
        def _f(x: Tensor) -> Tensor:
            return log_wide(x)
        return _f

    a = float(prior_mix)
    loga = math.log(max(a, 1e-12))
    log1a = math.log(max(1.0 - a, 1e-12))
    def _f(x: Tensor) -> Tensor:
        lg1 = gmm_logpdf(x, gmm_prior) + loga
        lg2 = log_wide(x)              + log1a
        m = torch.maximum(lg1, lg2)
        return m + torch.log(torch.exp(lg1 - m) + torch.exp(lg2 - m))
    return _f

# ===========================
# NUTS（只用于 xs；带掩码）
# ===========================
def logpost_xs_with_grad(
    xs: Tensor,
    *,
    xr: Tensor,
    Tp_obs: Tensor, Ts_obs: Tensor,
    t0: Tensor,
    sigmaP2: Tensor, sigmaS2: Tensor,
    log2pi: Tensor,
    log_prior_xs,  # 闭包
    maskP: Tensor, maskS: Tensor,
    Np: int, Ns: int,
) -> Tuple[Tensor, Tensor]:
    """
    计算 log posterior 与对 xs 的梯度（仅在 NUTS 内部用），带相位掩码。
    """
    xs = xs.requires_grad_(True)
    T_pred = model(xr, xs)  # (N,2) 带梯度
    tp_model = T_pred[:,0]
    ts_model = T_pred[:,1]

    ll = torch.tensor(0.0, dtype=Tp_obs.dtype, device=Tp_obs.device)
    if Np > 0:
        resP = Tp_obs[maskP] - t0 - tp_model[maskP]
        llP = -0.5 * torch.sum(resP * resP) / sigmaP2 - 0.5 * Np * (log2pi + torch.log(sigmaP2))
        ll = ll + llP
    if Ns > 0:
        resS = Ts_obs[maskS] - t0 - ts_model[maskS]
        llS = -0.5 * torch.sum(resS * resS) / sigmaS2 - 0.5 * Ns * (log2pi + torch.log(sigmaS2))
        ll = ll + llS

    logp = log_prior_xs(xs) + ll
    grad = torch.autograd.grad(logp, xs)[0]
    return logp.detach(), grad.detach()

def leapfrog(xs, r, eps, grad_fn):
    """
    单步 leapfrog；grad_fn 返回 (logp, grad)
    """
    logp, grad = grad_fn(xs)
    r_half = r + 0.5 * eps * grad
    xs_new = xs + eps * r_half
    logp_new, grad_new = grad_fn(xs_new)
    r_new = r_half + 0.5 * eps * grad_new
    H = -logp + 0.5 * torch.dot(r, r)
    H_new = -logp_new + 0.5 * torch.dot(r_new, r_new)
    return xs_new, r_new, logp_new, grad_new, H_new, H

def stop_criterion(xs_minus, xs_plus, r_minus, r_plus):
    """
    NUTS 停止条件：不再 U-turn
    """
    delta1 = torch.dot(xs_plus - xs_minus, r_minus)
    delta2 = torch.dot(xs_plus - xs_minus, r_plus)
    return (delta1 >= 0.0) and (delta2 >= 0.0)

def nuts_step(
    xs: Tensor,
    *,
    step_size: float,
    max_tree_depth: int,
    grad_fn,  # 返回 (logp, grad)
    generator: Optional[torch.Generator] = None,
    target_accept: float = 0.8,
):
    """
    单次 NUTS 构建树并返回新 xs、是否接受、以及新的 logp。
    简化实现，质量矩阵=I。
    """
    if generator is None:
        generator = torch.Generator(device=xs.device)

    r0 = torch.randn_like(xs)
    logp0, grad0 = grad_fn(xs)
    H0 = -logp0 + 0.5 * torch.dot(r0, r0)

    xs_minus = xs.clone()
    xs_plus  = xs.clone()
    r_minus  = r0.clone()
    r_plus   = r0.clone()
    xs_prop  = xs.clone()
    logp_prop = logp0.clone()
    s = True
    accept_stat = 0.0

    for depth in range(max_tree_depth):
        direction = 1 if torch.rand(()) < 0.5 else -1
        if direction == 1:
            xs_plus, r_plus, logp_new, grad_new, H_new, _ = leapfrog(xs_plus, r_plus, step_size, grad_fn)
        else:
            xs_minus, r_minus, logp_new, grad_new, H_new, _ = leapfrog(xs_minus, r_minus, -step_size, grad_fn)

        dH = H_new - H0
        if torch.isfinite(dH):
            prob = min(1.0, float(torch.exp(-dH).clamp(max=1e6)))
            if torch.rand(()) < prob:
                xs_prop = (xs_plus if direction == 1 else xs_minus).clone()
                logp_prop = logp_new.clone()
            accept_stat += min(1.0, float(torch.exp(-dH).clamp(max=1e6)))

        s = s and stop_criterion(xs_minus, xs_plus, r_minus, r_plus)
        if not s:
            break

    accepted = (xs_prop != xs).any()
    # 经验接受率（每层一次统计）
    accept_rate = float(accept_stat / max(1, depth + 1))
    return xs_prop, accepted, logp_prop, accept_rate
@torch.no_grad()
def forward_tps_multi(xs_batch: torch.Tensor,
                      xr: torch.Tensor,
                      event_ids: torch.Tensor) -> torch.Tensor:
    """
    xs_batch: [NC,3]   每个事件的震源
    xr:       [N,3]    台站坐标（所有观测拼接）
    event_ids: [N]     每条观测对应的事件索引 in [0, NC-1]

    返回:
      T_pred: [N,2]  列0=Tp, 列1=Ts
    """
    # xs_per_obs 通过索引映射到每条观测，不复制 xr
    xs_per_obs = xs_batch[event_ids]           # [N,3]
    tps = model(xr, xs_per_obs)
    
    return tps 




def _scatter_counts(mask: torch.Tensor, event_ids: torch.Tensor, NC: int) -> torch.Tensor:
    # 统计每事件的观测条数（仅在 mask=True 的样本）
    ones = mask.to(torch.float32)
    return torch.zeros(NC, device=mask.device, dtype=torch.float32).scatter_add_(0, event_ids, ones)

def _scatter_sum(vals: torch.Tensor, mask: torch.Tensor, event_ids: torch.Tensor, NC: int) -> torch.Tensor:
    # 统计每事件的加和（仅 mask=True）
    v = torch.where(mask, vals.to(torch.float32), torch.tensor(0.0, device=vals.device, dtype=torch.float32))
    return torch.zeros(NC, device=vals.device, dtype=torch.float32).scatter_add_(0, event_ids, v)

def _scatter_sum_sq(vals: torch.Tensor, mask: torch.Tensor, event_ids: torch.Tensor, NC: int) -> torch.Tensor:
    v = torch.where(mask, (vals.to(torch.float32) ** 2), torch.tensor(0.0, device=vals.device, dtype=torch.float32))
    return torch.zeros(NC, device=vals.device, dtype=torch.float32).scatter_add_(0, event_ids, v)


def log_lik_per_event(
    T_pred: torch.Tensor,             # [N,2]
    Tp_obs: torch.Tensor, Ts_obs: torch.Tensor,  # [N]
    t0: torch.Tensor,                 # [NC]
    sigmaP2: torch.Tensor, sigmaS2: torch.Tensor,# [NC]
    log2pi: torch.Tensor,
    maskP: torch.Tensor, maskS: torch.Tensor,    # [N] bool
    event_ids: torch.Tensor,          # [N] long
    NC: int,
    lambdaP: Optional[torch.Tensor] = None,      # [N] or None
    lambdaS: Optional[torch.Tensor] = None,      # [N] or None
) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
    """
    若 lambdaP/lambdaS 不为 None，则按 Student-t scale mixture 做加权 RSS：
        rss = sum_i lambda_i * r_i^2
        N   = sum_i 1（仍然是样本数）
    """
    tp_model = T_pred[:, 0]
    ts_model = T_pred[:, 1]
    
    if lambdaP is None:
        lambdaP_eff = torch.ones_like(Tp_obs, dtype=torch.float32)
    else:
        lambdaP_eff = lambdaP.to(torch.float32)

    if lambdaS is None:
        lambdaS_eff = torch.ones_like(Ts_obs, dtype=torch.float32)
    else:
        lambdaS_eff = lambdaS.to(torch.float32)

    # P 残差与统计
    resP = Tp_obs - tp_model - t0[event_ids]
    # 加权残差平方和
    resP_w2 = lambdaP_eff * (resP.to(torch.float32) ** 2)
    rssP_e = _scatter_sum(resP_w2, maskP, event_ids, NC)     # 注意：用 _scatter_sum 而不是 _scatter_sum_sq
    Np_e   = _scatter_counts(maskP, event_ids, NC)           # 样本数还是未加权的

    # S 残差
    resS = Ts_obs - ts_model - t0[event_ids]
    resS_w2 = lambdaS_eff * (resS.to(torch.float32) ** 2)
    rssS_e = _scatter_sum(resS_w2, maskS, event_ids, NC)
    Ns_e   = _scatter_counts(maskS, event_ids, NC)

    sumP_e = _scatter_sum(resP, maskP, event_ids, NC)  
    sumS_e = _scatter_sum(resS, maskS, event_ids, NC)


    llP_e = torch.zeros(NC, device=Tp_obs.device, dtype=torch.float32)
    llS_e = torch.zeros(NC, device=Tp_obs.device, dtype=torch.float32)

    # P log-lik：近似仍用 Np_e 作为有效样本数（严格 Student-t 常数项会变，但差别在常数）
    validP = Np_e > 0
    if validP.any():
        llP_e[validP] = -0.5 * (rssP_e[validP] / sigmaP2[validP]) \
                        -0.5 * Np_e[validP] * (log2pi + torch.log(sigmaP2[validP]))

    validS = Ns_e > 0
    if validS.any():
        llS_e[validS] = -0.5 * (rssS_e[validS] / sigmaS2[validS]) \
                        -0.5 * Ns_e[validS] * (log2pi + torch.log(sigmaS2[validS]))

    ll_e = llP_e + llS_e
    return ll_e, Np_e, Ns_e, sumP_e, sumS_e, rssP_e, rssS_e



def _gamma_sample_shape_rate(shape: torch.Tensor, rate: torch.Tensor) -> torch.Tensor:
    """
    Sample from Gamma(shape, rate) with shape broadcasting to rate.
    - CUDA/CPU: sample on the same device.
    - MPS: move to CPU to sample (since gamma sampling ops are not implemented on MPS),
           then move back.
    """
    # Ensure tensors
    if shape.ndim == 0:
        shape_b = shape.expand_as(rate)
    else:
        shape_b = shape

    if rate.is_mps:
        # --- MPS workaround: sample on CPU ---
        shape_cpu = shape_b.detach().to("cpu")
        rate_cpu  = rate.detach().to("cpu")
        # torch.distributions.Gamma uses (concentration=shape, rate=rate)
        g = torch.distributions.Gamma(shape_cpu, rate_cpu)
        samp_cpu = g.sample()  # [N]
        return samp_cpu.to(device=rate.device, dtype=rate.dtype)
    else:
        g = torch.distributions.Gamma(shape_b, rate)
        return g.sample()
def _beta_sample(a: torch.Tensor, b: torch.Tensor, *, eps: float = 1e-6) -> torch.Tensor:
    """
    Sample Beta(a,b) using Gamma trick:
        X ~ Gamma(a, 1), Y ~ Gamma(b, 1),  Beta = X/(X+Y)

    Works on CPU/CUDA; on MPS uses your _gamma_sample_shape_rate CPU fallback.
    a,b can be scalar tensors or broadcastable tensors.
    """
    one = torch.ones_like(a if a.ndim > 0 else b)
    # Use rate=1
    x = _gamma_sample_shape_rate(a, one)
    y = _gamma_sample_shape_rate(b, one)
    return (x / (x + y + eps)).clamp(eps, 1.0 - eps)

def log_student_t0(r: torch.Tensor, sig2: torch.Tensor, nu: float) -> torch.Tensor:
    """
    log pdf of Student-t with df=nu, mean 0, scale sigma (variance = sig2*nu/(nu-2) if nu>2).
    Parameterization: r = sigma * t_nu
    pdf = Gamma((nu+1)/2)/[Gamma(nu/2)*sqrt(nu*pi)*sigma] * (1 + (r^2)/(nu*sigma^2))^(-(nu+1)/2)
    """
    nu_t = torch.tensor(float(nu), device=r.device, dtype=r.dtype)
    # logC = lgamma((nu+1)/2) - lgamma(nu/2) - 0.5*log(nu*pi) - log(sigma)
    logC = (torch.lgamma((nu_t + 1.0) / 2.0) -
            torch.lgamma(nu_t / 2.0) -
            0.5 * (torch.log(nu_t) + torch.log(torch.tensor(math.pi, device=r.device, dtype=r.dtype))) -
            0.5 * torch.log(sig2))
    quad = 1.0 + (r * r) / (nu_t * sig2)
    return logC - 0.5 * (nu_t + 1.0) * torch.log(quad)

def update_lambda_student_t(
    Tp_obs: torch.Tensor,
    Ts_obs: torch.Tensor,
    T_pred_curr: torch.Tensor,    # [N,2]
    t0: torch.Tensor,             # [NC]
    sigmaP2: torch.Tensor,        # [NC]
    sigmaS2: torch.Tensor,        # [NC]
    event_ids: torch.Tensor,      # [N]
    maskP: torch.Tensor,
    maskS: torch.Tensor,
    lambdaP: torch.Tensor,
    lambdaS: torch.Tensor,
    nuP: float,
    nuS: float,
    eps_rate: float = 1e-12,
):
    """
    Student-t scale mixture:
      r | λ,σ² ~ N(0, σ²/λ)
      λ ~ Gamma(ν/2, ν/2)  (shape, rate)
    Gibbs:
      λ | r,σ² ~ Gamma((ν+1)/2, (ν + r²/σ²)/2)

    MPS note:
      Gamma sampling kernels are not implemented on MPS in current PyTorch builds,
      so we sample on CPU when device is MPS, then copy back.
    """
    device = Tp_obs.device
    dtype  = Tp_obs.dtype

    # Coerce key tensors to same device/dtype (avoid implicit CPU scalars)
    event_ids = event_ids.to(device=device)
    maskP = maskP.to(device=device)
    maskS = maskS.to(device=device)

    tp_model = T_pred_curr[:, 0].to(device=device, dtype=dtype)
    ts_model = T_pred_curr[:, 1].to(device=device, dtype=dtype)

    t0 = t0.to(device=device, dtype=dtype)
    sigmaP2 = sigmaP2.to(device=device, dtype=dtype)
    sigmaS2 = sigmaS2.to(device=device, dtype=dtype)

    sigmaP2_obs = sigmaP2[event_ids]  # [N]
    sigmaS2_obs = sigmaS2[event_ids]  # [N]

    nuP_t = torch.tensor(nuP, device=device, dtype=dtype)
    nuS_t = torch.tensor(nuS, device=device, dtype=dtype)

    # ---- P phase ----
    idxP = maskP
    if bool(idxP.any()):
        resP = Tp_obs[idxP].to(device=device, dtype=dtype) - t0[event_ids[idxP]] - tp_model[idxP]
        r2P  = resP * resP

        shapeP = 0.5 * (nuP_t + 1.0)                           # scalar tensor
        rateP  = 0.5 * (nuP_t + r2P / sigmaP2_obs[idxP])       # [Np]
        rateP  = rateP.clamp_min(eps_rate)

        lamP = _gamma_sample_shape_rate(shapeP, rateP)
        lambdaP = lambdaP.to(device=device, dtype=dtype)
        lambdaP[idxP] = lamP

    # ---- S phase ----
    idxS = maskS
    if bool(idxS.any()):
        resS = Ts_obs[idxS].to(device=device, dtype=dtype) - t0[event_ids[idxS]] - ts_model[idxS]
        r2S  = resS * resS

        shapeS = 0.5 * (nuS_t + 1.0)
        rateS  = 0.5 * (nuS_t + r2S / sigmaS2_obs[idxS])
        rateS  = rateS.clamp_min(eps_rate)

        lamS = _gamma_sample_shape_rate(shapeS, rateS)
        lambdaS = lambdaS.to(device=device, dtype=dtype)
        lambdaS[idxS] = lamS

    return lambdaP, lambdaS



import math
from typing import Dict, Optional
import torch

@torch.no_grad()
def gibbs_mh_location_multi(
    Tp_obs: torch.Tensor, Ts_obs: torch.Tensor,   # [N]
    xr: torch.Tensor,                             # [N,3]
    event_ids: torch.Tensor,                      # [N] long in [0..NC-1]
    NC: int,
    n_samples: int = 4000, burn: int = 1000, thin: int = 1,
    xs_init: Optional[torch.Tensor] = None,       # [NC,3] or None
    t0_init: float = 0.0,
    alpha0: float = 1e-2, beta0: float = 1e-2,
    sigmaP_init: float = 0.1, sigmaS_init: float = 0.1,
    prop_scale: float = 0.5,
    *,
    device_: Optional[torch.device] = None,
    dtype: torch.dtype = torch.float32,
    verbose: bool = True,
    generator: Optional[torch.Generator] = None,
    # 先验（仅用于 log_prior_xs）
    gmm_prior: Optional[Dict[str, torch.Tensor]] = None,
    prior_mix: Optional[float] = None,
    wide_sigma: float = 1000.0,
    # RW 步长自适应
    adapt_steps: int = 1000,
    target_accept_rw: float = 0.3,
    adapt_eta: float = 0.05,
    # Student-t
    use_student_t: bool = True,

    # ===== 新增：两分量污染模型参数 =====
    a_pi: float = 8.0,
    b_pi: float = 2.0,
    piP_init: float = 0.9,
    piS_init: float = 0.9,
    sigma_out_P: float = 15.0,   # outlier std for P (seconds)
    sigma_out_S: float = 20.0,  # outlier std for S (seconds)
) -> Dict[str, torch.Tensor]:

    device_eff = device_ if device_ is not None else device
    if generator is None:
        generator = torch.Generator(device=device_eff)

    # ----------------------------
    # 0) dtype / shape
    # ----------------------------
    Tp_obs = torch.as_tensor(Tp_obs, dtype=dtype, device=device_eff).view(-1)
    Ts_obs = torch.as_tensor(Ts_obs, dtype=dtype, device=device_eff).view(-1)
    xr     = torch.as_tensor(xr,     dtype=dtype, device=device_eff)
    event_ids = torch.as_tensor(event_ids, dtype=torch.long, device=device_eff).view(-1)

    N = Tp_obs.numel()
    assert Ts_obs.shape == (N,) and xr.shape == (N, 3)
    assert event_ids.shape == (N,) and int(event_ids.max().item()) < NC

    # masks (observed)
    maskP, maskS, _, _ = build_phase_masks(Tp_obs, Ts_obs, missing_value=MISSING_VAL)

    # Student-t df
    nuP = 4
    nuS = 4

    # latent lambdas (for Student-t mixture)
    lambdaP = torch.ones_like(Tp_obs, dtype=dtype, device=device_eff)
    lambdaS = torch.ones_like(Ts_obs, dtype=dtype, device=device_eff)

    # latent good/outlier indicators (only meaningful where mask is True)
    zP = torch.ones_like(Tp_obs, dtype=torch.bool, device=device_eff)
    zS = torch.ones_like(Ts_obs, dtype=torch.bool, device=device_eff)

    # mixture probabilities (global; you can extend to per-event if needed)
    piP = torch.tensor(float(piP_init), dtype=dtype, device=device_eff)
    piS = torch.tensor(float(piS_init), dtype=dtype, device=device_eff)

    # fixed outlier variances
    sigOutP2 = torch.tensor(float(sigma_out_P**2), dtype=dtype, device=device_eff)
    sigOutS2 = torch.tensor(float(sigma_out_S**2), dtype=dtype, device=device_eff)

    # ----------------------------
    # 1) init xs
    # ----------------------------
    if xs_init is None:
        xs = torch.empty((NC, 3), device=device_eff, dtype=dtype)
        xr_global_mean = xr.mean(dim=0)
        valid_obs = maskP | maskS
        for e in range(NC):
            idx_e = (event_ids == e) & valid_obs
            if idx_e.any():
                xs[e] = xr[idx_e].mean(dim=0)
            else:
                idx_e2 = (event_ids == e)
                xs[e] = xr[idx_e2].mean(dim=0) if idx_e2.any() else xr_global_mean
        xs = xs + torch.tensor([0.0, 0.0, 5.0], device=device_eff, dtype=dtype).view(1, 3)
    else:
        xs = torch.as_tensor(xs_init, dtype=dtype, device=device_eff).view(NC, 3)
    bais = 20
    base1 = torch.tensor([bais, bais, 2.0], device=device_eff, dtype=dtype).view(1, 3)
    base2 = torch.tensor([bais, -bais, -2.0], device=device_eff, dtype=dtype).view(1, 3)
    base3 = torch.tensor([-bais, -bais, 2.0], device=device_eff, dtype=dtype).view(1, 3)
    base4 = torch.tensor([-bais, bais, -2.0], device=device_eff, dtype=dtype).view(1, 3)
    t0      = torch.full((NC,), float(t0_init),      dtype=dtype, device=device_eff)
    bases = [base1, base2, base3, base4]
    btime = [-2, 0, 2, 4]
    #xs = torch.zeros([NC, 3], dtype=dtype, device=device_eff)
    for k in range(4):
        xs[k*4:k*4+4, :] += bases[k]
        t0[k*4:k*4+4] += btime[k]

    xs_init = xs.clone()
    # init t0, sigma^2 (per-event)
    t0      = torch.full((NC,), float(t0_init), dtype=dtype, device=device_eff)
    sigmaP2 = torch.full((NC,), float(sigmaP_init**2), dtype=dtype, device=device_eff)
    sigmaS2 = torch.full((NC,), float(sigmaS_init**2), dtype=dtype, device=device_eff)

    # sample buffers
    M = (n_samples - burn) // max(1, thin)
    xs_samples     = torch.empty((M, NC, 3), dtype=dtype, device=device_eff)
    t0_samples     = torch.empty((M, NC),     dtype=dtype, device=device_eff)
    sigmaP_samples = torch.empty((M, NC),     dtype=dtype, device=device_eff)
    sigmaS_samples = torch.empty((M, NC),     dtype=dtype, device=device_eff)
    piP_samples    = torch.empty((M,),        dtype=dtype, device=device_eff)
    piS_samples    = torch.empty((M,),        dtype=dtype, device=device_eff)

    # constants
    log2pi = torch.tensor(math.log(2.0 * math.pi), dtype=dtype, device=device_eff)

    # prior closure
    log_prior_xs = make_log_prior_xs(
        gmm_prior=gmm_prior,
        wide_sigma=wide_sigma,
        prior_mix=prior_mix,
        device=device_eff,
        dtype=dtype,
    )

    # RW step scale (per-event)
    log_prop_scale = torch.full((NC,), math.log(max(prop_scale, 1e-6)), dtype=dtype, device=device_eff)
    accept_count_e = torch.zeros(NC, dtype=torch.int32, device=device_eff)
    total_prop = 0
    sample_idx = 0

    # forward once
    T_pred_curr = forward_tps_multi(xs, xr, event_ids)  # [N,2]

    # ----------------------------
    # helper: scatter sum for values with boolean mask
    # ----------------------------
    def scatter_sum_masked(val: torch.Tensor, m: torch.Tensor) -> torch.Tensor:
        out = torch.zeros((NC,), dtype=dtype, device=device_eff)
        if m.any():
            out.scatter_add_(0, event_ids[m], val[m])
        return out

    def scatter_count_masked(m: torch.Tensor) -> torch.Tensor:
        return scatter_sum_masked(torch.ones_like(Tp_obs, dtype=dtype), m)

    # ----------------------------
    # helper: log N(r | 0, var)  (elementwise)
    # ----------------------------
    def log_norm0(r: torch.Tensor, var: torch.Tensor) -> torch.Tensor:
        # var can be scalar or [N]
        return -0.5 * (log2pi + torch.log(var) + (r * r) / var)

    # ----------------------------
    # helper: log N(r | 0, sigma^2/lambda) for "good" (conditional Student-t)
    # ----------------------------
    def log_good_cond(r: torch.Tensor, sig2_e: torch.Tensor, lam: torch.Tensor) -> torch.Tensor:
        # sig2_e: [N] (picked by event), lam: [N]
        # log N(0, sig2/lam) = 0.5*log(lam) -0.5*(log(2π)+log(sig2)+lam*r^2/sig2)
        return 0.5 * torch.log(lam) - 0.5 * (log2pi + torch.log(sig2_e) + (lam * r * r) / sig2_e)

    # ----------------------------
    # helper: joint log-likelihood per-event given current z, pi, lambdas
    #          includes log p(z|pi)
    # ----------------------------
    def log_joint_per_event(
        T_pred: torch.Tensor,
        xs_: torch.Tensor,
        t0_: torch.Tensor,
        sigmaP2_: torch.Tensor,
        sigmaS2_: torch.Tensor,
        lambdaP_: torch.Tensor,
        lambdaS_: torch.Tensor,
        zP_: torch.Tensor,
        zS_: torch.Tensor,
        piP_: torch.Tensor,
        piS_: torch.Tensor,
    ) -> torch.Tensor:
        tp_model = T_pred[:, 0]
        ts_model = T_pred[:, 1]

        resP = Tp_obs - t0_[event_ids] - tp_model
        resS = Ts_obs - t0_[event_ids] - ts_model

        sigP2_i = sigmaP2_[event_ids]  # [N]
        sigS2_i = sigmaS2_[event_ids]

        llP = torch.zeros((N,), dtype=dtype, device=device_eff)
        llS = torch.zeros((N,), dtype=dtype, device=device_eff)

        # only where observed
        if maskP.any():
            goodP = maskP & zP_
            badP  = maskP & (~zP_)
            if goodP.any():
                if use_student_t:
                    llP[goodP] = log_good_cond(resP[goodP], sigP2_i[goodP], lambdaP_[goodP])
                else:
                    llP[goodP] = log_norm0(resP[goodP], sigP2_i[goodP])
                llP[goodP] += torch.log(piP_.clamp(min=1e-6))
            if badP.any():
                llP[badP] = log_norm0(resP[badP], sigOutP2)
                llP[badP] += torch.log((1.0 - piP_).clamp(min=1e-6))

        if maskS.any():
            goodS = maskS & zS_
            badS  = maskS & (~zS_)
            if goodS.any():
                if use_student_t:
                    llS[goodS] = log_good_cond(resS[goodS], sigS2_i[goodS], lambdaS_[goodS])
                else:
                    llS[goodS] = log_norm0(resS[goodS], sigS2_i[goodS])
                llS[goodS] += torch.log(piS_.clamp(min=1e-6))
            if badS.any():
                llS[badS] = log_norm0(resS[badS], sigOutS2)
                llS[badS] += torch.log((1.0 - piS_).clamp(min=1e-6))

        ll_obs = llP + llS
        ll_e = scatter_sum_masked(ll_obs, maskP | maskS)  # sum all obs contributions per event
        # add xs prior (per-event)
        return ll_e + log_prior_xs(xs_)

    # ----------------------------
    # helper: update zP / zS using p(z=1|...) ∝ pi*good  vs (1-pi)*out
    # ----------------------------
    def update_z_two_component(
        T_pred: torch.Tensor,
        t0_: torch.Tensor,
        sigmaP2_: torch.Tensor,
        sigmaS2_: torch.Tensor,
        lambdaP_: torch.Tensor,
        lambdaS_: torch.Tensor,
        piP_: torch.Tensor,
        piS_: torch.Tensor,
        zP_: torch.Tensor,
        zS_: torch.Tensor,
    ):
        tp_model = T_pred[:, 0]
        ts_model = T_pred[:, 1]
        resP = Tp_obs - t0_[event_ids] - tp_model
        resS = Ts_obs - t0_[event_ids] - ts_model
        sigP2_i = sigmaP2_[event_ids]
        sigS2_i = sigmaS2_[event_ids]

        # P
        if maskP.any():
            m = maskP
            if use_student_t:
                log_good = log_student_t0(resP[m], sigP2_i[m], nuP)
            else:
                log_good = log_norm0(resP[m], sigP2_i[m])

            log_out = log_norm0(resP[m], sigOutP2)

            logp1 = torch.log(piP_.clamp(min=1e-6)) + log_good
            logp0 = torch.log((1.0 - piP_).clamp(min=1e-6)) + log_out
            # p(z=1) = exp(logp1 - logsumexp(logp1,logp0))
            mmax = torch.maximum(logp1, logp0)
            denom = mmax + torch.log(torch.exp(logp1 - mmax) + torch.exp(logp0 - mmax))
            p1 = torch.exp(logp1 - denom).clamp(1e-6, 1 - 1e-6)
            u = torch.rand_like(p1)
            z_new = u < p1
            zP_[m] = z_new

        # S
        if maskS.any():
            m = maskS
            if use_student_t:
                log_good = log_student_t0(resS[m], sigS2_i[m], nuS)
            else:
                log_good = log_norm0(resS[m], sigS2_i[m])

            log_out = log_norm0(resS[m], sigOutS2)

            logp1 = torch.log(piS_.clamp(min=1e-6)) + log_good
            logp0 = torch.log((1.0 - piS_).clamp(min=1e-6)) + log_out
            mmax = torch.maximum(logp1, logp0)
            denom = mmax + torch.log(torch.exp(logp1 - mmax) + torch.exp(logp0 - mmax))
            p1 = torch.exp(logp1 - denom).clamp(1e-6, 1 - 1e-6)
            u = torch.rand_like(p1)
            z_new = u < p1
            zS_[m] = z_new

    # ----------------------------
    # init current joint logpost per event
    # ----------------------------
    curr_logpost_e = log_joint_per_event(
        T_pred_curr, xs, t0, sigmaP2, sigmaS2,
        lambdaP, lambdaS, zP, zS, piP, piS
    )

    if verbose:
        print("Start sampling (RW + Student-t + Good/Outlier mixture)...")

    for k in range(n_samples):
        if verbose and (k % 50 == 0):
            print(f"[iter {k}] piP={piP.item():.6f} piS={piS.item():.6f} "
                f"badP={(maskP & (~zP)).float().mean().item():.3f} "
                f"badS={(maskS & (~zS)).float().mean().item():.3f}")


        # ===== A0. update lambdas (Student-t latent) =====
        if use_student_t:
            update_lambda_student_t(
                Tp_obs, Ts_obs,
                T_pred_curr,
                t0, sigmaP2, sigmaS2,
                event_ids,
                maskP, maskS,
                lambdaP, lambdaS,
                nuP, nuS,
            )

        # ===== A1. update zP/zS (good/outlier indicators) =====
        update_z_two_component(
            T_pred_curr, t0, sigmaP2, sigmaS2,
            lambdaP, lambdaS, piP, piS, zP, zS
        )


        # ===== A2. update piP/piS (Beta posterior) =====
        nP_good = (maskP & zP).sum().to(dtype)
        nP_bad  = (maskP & (~zP)).sum().to(dtype)
        nS_good = (maskS & zS).sum().to(dtype)
        nS_bad  = (maskS & (~zS)).sum().to(dtype)

        aP = torch.tensor(a_pi, device=device_eff, dtype=dtype) + nP_good
        bP = torch.tensor(b_pi, device=device_eff, dtype=dtype) + nP_bad
        aS = torch.tensor(a_pi, device=device_eff, dtype=dtype) + nS_good
        bS = torch.tensor(b_pi, device=device_eff, dtype=dtype) + nS_bad

        # Beta via Gamma trick, MPS-safe (uses your CPU fallback)
        piP = _beta_sample(aP, bP, eps=1e-6)
        piS = _beta_sample(aS, bS, eps=1e-6)


        # ===== B. RW-MH update xs (per-event, vectorized) =====
        total_prop += 1

        steps = torch.randn((NC, 3), dtype=dtype, device=device_eff) \
                * log_prop_scale.exp().unsqueeze(1)
        xs_prop = xs + steps

        T_pred_prop = forward_tps_multi(xs_prop, xr, event_ids)

        prop_logpost_e = log_joint_per_event(
            T_pred_prop, xs_prop, t0, sigmaP2, sigmaS2,
            lambdaP, lambdaS, zP, zS, piP, piS
        )

        log_alpha_e = prop_logpost_e - curr_logpost_e
        u = torch.rand((NC,), dtype=dtype, device=device_eff).log()
        accept_e = (u < log_alpha_e)

        if accept_e.any():
            xs[accept_e] = xs_prop[accept_e]
            curr_logpost_e[accept_e] = prop_logpost_e[accept_e]
            accept_count_e[accept_e] += 1

            accept_obs = accept_e[event_ids]
            T_pred_curr[accept_obs] = T_pred_prop[accept_obs]

        # RW adapt (建议用 accept indicator 更稳)
        if k < adapt_steps:
            log_prop_scale += adapt_eta * (accept_e.to(dtype) - target_accept_rw)

        # ===== C. update t0 (condition on GOOD only; z selects the component) =====
        tp_model = T_pred_curr[:, 0]
        ts_model = T_pred_curr[:, 1]

        # effective masks: observed & good
        mP_good = maskP & zP
        mS_good = maskS & zS

        # Σ λ_i * (T_obs - T_model)
        sumP_nominal = scatter_sum_masked(lambdaP * (Tp_obs - tp_model), mP_good) if use_student_t \
                       else scatter_sum_masked((Tp_obs - tp_model), mP_good)
        sumS_nominal = scatter_sum_masked(lambdaS * (Ts_obs - ts_model), mS_good) if use_student_t \
                       else scatter_sum_masked((Ts_obs - ts_model), mS_good)

        # Σ λ_i  (or counts)
        Np_eff = scatter_sum_masked(lambdaP, mP_good) if use_student_t else scatter_count_masked(mP_good)
        Ns_eff = scatter_sum_masked(lambdaS, mS_good) if use_student_t else scatter_count_masked(mS_good)

        denom = torch.zeros(NC, dtype=dtype, device=device_eff)
        num   = torch.zeros(NC, dtype=dtype, device=device_eff)

        validP = Np_eff > 0
        if validP.any():
            denom[validP] += Np_eff[validP] / sigmaP2[validP]
            num[validP]   += sumP_nominal[validP] / sigmaP2[validP]

        validS = Ns_eff > 0
        if validS.any():
            denom[validS] += Ns_eff[validS] / sigmaS2[validS]
            num[validS]   += sumS_nominal[validS] / sigmaS2[validS]

        upd_mask = denom > 0
        if upd_mask.any():
            var_t0  = torch.zeros_like(denom)
            mean_t0 = torch.zeros_like(denom)
            var_t0[upd_mask]  = 1.0 / denom[upd_mask]
            mean_t0[upd_mask] = var_t0[upd_mask] * num[upd_mask]
            noise = torch.zeros_like(mean_t0)
            noise[upd_mask] = torch.randn((int(upd_mask.sum().item()),), dtype=dtype,
                                          device=device_eff)
            t0[upd_mask] = mean_t0[upd_mask] + noise[upd_mask] * var_t0[upd_mask].sqrt()

        # ===== D. update sigmaP2 / sigmaS2 (InvGamma; GOOD only) =====
        resP = Tp_obs - t0[event_ids] - tp_model
        resS = Ts_obs - t0[event_ids] - ts_model

        if use_student_t:
            rssP_e = scatter_sum_masked(lambdaP * (resP ** 2), mP_good)
            rssS_e = scatter_sum_masked(lambdaS * (resS ** 2), mS_good)
            Np_eff = scatter_sum_masked(lambdaP, mP_good)
            Ns_eff = scatter_sum_masked(lambdaS, mS_good)
        else:
            rssP_e = scatter_sum_masked(resP ** 2, mP_good)
            rssS_e = scatter_sum_masked(resS ** 2, mS_good)
            Np_eff = scatter_count_masked(mP_good)
            Ns_eff = scatter_count_masked(mS_good)

        alphaP = torch.full((NC,), alpha0, dtype=dtype, device=device_eff)
        betaP  = torch.full((NC,), beta0,  dtype=dtype, device=device_eff)
        alphaS = torch.full((NC,), alpha0, dtype=dtype, device=device_eff)
        betaS  = torch.full((NC,), beta0,  dtype=dtype, device=device_eff)

        validP = Np_eff > 0
        validS = Ns_eff > 0
        if validP.any():
            alphaP[validP] += 0.5 * Np_eff[validP]
            betaP[validP]  += 0.5 * rssP_e[validP]
        if validS.any():
            alphaS[validS] += 0.5 * Ns_eff[validS]
            betaS[validS]  += 0.5 * rssS_e[validS]

        sigmaP2 = sample_invgamma_batch(alphaP, betaP)
        sigmaS2 = sample_invgamma_batch(alphaS, betaS)

        # ===== E. recompute current joint logpost per-event =====
        curr_logpost_e = log_joint_per_event(
            T_pred_curr, xs, t0, sigmaP2, sigmaS2,
            lambdaP, lambdaS, zP, zS, piP, piS
        )

        # ===== F. record =====
        if k >= burn and ((k - burn) % thin == 0):
            xs_samples[sample_idx]     = xs
            t0_samples[sample_idx]     = t0
            sigmaP_samples[sample_idx] = sigmaP2.sqrt()
            sigmaS_samples[sample_idx] = sigmaS2.sqrt()
            piP_samples[sample_idx]    = piP
            piS_samples[sample_idx]    = piS
            sample_idx += 1

    accept_rate_e = accept_count_e.to(torch.float32) / max(1, total_prop)

    return {
        "xs_init": xs_init,
        "xs_samples": xs_samples,                 # [M, NC, 3]
        "t0_samples": t0_samples,                 # [M, NC]
        "sigmaP_samples": sigmaP_samples,         # [M, NC]
        "sigmaS_samples": sigmaS_samples,         # [M, NC]
        "piP_samples": piP_samples,               # [M]
        "piS_samples": piS_samples,               # [M]
        "accept_rate_per_event": accept_rate_e,   # [NC]
        "final_prop_scale_per_event": log_prop_scale.exp(),  # [NC]
        # 如果你想调试/评估鲁棒性，可以把最终 z 输出（注意这是最后一次迭代的 z）
        "zP_final": zP,
        "zS_final": zS,
        "piP_final": piP,
        "piS_final": piS,
    }


import datetime 
import numpy as np 
import pickle 

import re

def starts_with_int(s: str) -> bool:
    # ^           开头
    # [+-]?       可选正负号
    # \d+         一段数字（整数）
    # (?!\d|\.)   后面不能再接数字或小数点，保证是“整型整数”边界
    pattern = r'^[+-]?\d+(?!\d|\.)'
    return re.match(pattern, s) is not None
def read_station_file(file_path='data/station.txt'):
    stations = {}
    with open(file_path, 'r', encoding='utf-8') as f:
        for line in f:
            if not line.strip() or line.startswith("#"):
                continue
            parts = line.split()
            network = parts[0]
            station_code = parts[1]
            network_id = parts[2]
            lon = float(parts[2])
            lat = float(parts[3])
            ele = float(parts[4]) 
            stations[f"{network}{station_code}"] = np.array([lon, lat, -ele/1000])
    return stations 


def read_event_file(file_path):
    with open("data/vel.py.model", 'rb') as f:
        grid_vp, grid_vs, grid_x, grid_y, grid_z, proj, *rest = pickle.load(f)
    events = []
    stloc = read_station_file() 
    with open(file_path, 'r', encoding='utf-8') as f:
        lines = [line.strip() for line in f if line.strip()]
    #10950   2022 09 15 15:57:18.825    57438.825  0.2910 29.8570 101.8684   6.00    0.0   0.0  11  11  22   8  95.26
    #SC      JLO     P   57455.8100   16.9846 0.00e+00 -0.1447   0.6710  19.8026
    #SC     TGXJ     S   57464.0100   25.1846 0.00e+00 -0.6463   0.2680  23.6516
    #SC    KDXDQ     P   57445.9200    7.0946 0.00e+00 -0.3215   0.9550 118.9069
    #SC     YLXJ     S   57450.2100   11.3846 0.00e+00  0.1281   0.3880 186.7945
    #SC     YLXJ     P   57445.4800    6.6546 0.00e+00  0.1457   0.8010 186.7945
    i = 0
    while i < len(lines):
        # 读取事件头信息
        #print(starts_with_int(lines[i]), lines[i])
        if starts_with_int(lines[i]):
            print(lines[i])
            header = lines[i][:].strip().split()[1:]
            # 时间
            try:
                origin_time = datetime.datetime.strptime(" ".join(header[0:4]), "%Y %m %d %H %M %S.%f")
            except:
                origin_time = datetime.datetime.strptime(" ".join(header[0:4]).replace("60", "59"), "%Y %m %d %H:%M:%S.%f")
            lat = float(header[6])
            lon = float(header[7])
            depth = float(header[8])
            mag1 = float(header[9])
            mag2 = float(header[10])
            base_delta = float(header[4])  # 这个是儒略日
            # 后面的几个是未知或保留字段
            event_info = {
                "origin_time": origin_time,
                "latitude": lat,
                "longitude": lon,
                "depth": depth,
                "mag1": mag1,
                "mag2": mag2,
                "phases": []
            }

            # 读取震相行
            i += 1
            while i < len(lines) and not starts_with_int(lines[i]):
                parts = lines[i].split()
                station = "".join(parts[:2])
                travel_time = float(parts[4])# - base_delta 
                weight = float(parts[6])
                phase = parts[2]
                event_info["phases"].append({
                    "station": station,
                    "travel_time": travel_time,
                    "weight": weight,
                    "phase": phase
                })
                i += 1

            events.append(event_info)
        else:
            i += 1
    loc_events = []
    all_events = []
    for e in events: 
        #print("事件时间:", e["origin_time"])
        #print("位置: lat=%.4f lon=%.4f depth=%.2f" % (e["latitude"], e["longitude"], e["depth"]))
        #print("震相数据:")
        #locinfo = []
        tstr = e["origin_time"].strftime("%Y%m%d")
        #if "0905" not in tstr:
        #    continue
        x, y = proj(e["longitude"], e["latitude"]) 
        z = e["depth"]
        all_events.append([x, y, z])
        #events_loc.append(e)
        stations = []
        for p in e["phases"]:
            stations.append(p["station"])

        set_stations = list(set(stations))
        st2id = {}
        for i, station in enumerate(set_stations):
            st2id[station] = i
        rcv = np.zeros([len(set_stations), 3])
        T_p = np.ones([len(set_stations)]) * MISSING_VAL
        T_s = np.ones([len(set_stations)]) * MISSING_VAL
        for st in set_stations:
            if st not in stloc:
                continue
            loc = stloc[st]
            x, y = proj(loc[0], loc[1])
            z = loc[2] 
            if x < -1000 or x > 1000:
                continue
            if y < -1000 or y > 1000:
                continue
            if z < -10 or z > 100:
                continue
            rcv[st2id[st], :] = np.array([x, y, z]) 
    
        for p in e["phases"]: 
            if p['station'] not in stloc:
                continue
            loc = stloc[p['station']]
            x, y = proj(loc[0], loc[1])
            z = loc[2] 
            if x < -1000 or x > 1000:
                continue
            if y < -1000 or y > 1000:
                continue
            if z < -10 or z > 100:
                continue
            if p["phase"] == "P":
                T_p[st2id[p["station"]]] = p['travel_time'] 
            elif p["phase"] == "S":
                T_s[st2id[p["station"]]] = p['travel_time']
        locinfo = {
            "rcv":torch.tensor(rcv.astype(np.float32), device=device).float(), 
            "T_p":torch.tensor(T_p.astype(np.float32), device=device).float(),
            "T_s":torch.tensor(T_s.astype(np.float32), device=device).float(),
        }
        loc_events.append({
            "etime":e["origin_time"], 
            "locinfo":locinfo, 
            "mag1":e["mag1"],
            "mag2":e["mag2"],
        })
    all_events = np.array(all_events).astype(np.float32)
    all_events = torch.tensor(all_events, device=device).float()
    return loc_events, all_events, proj 


btime = datetime.datetime(2022, 9, 5, 0, 0)
begin = datetime.datetime.strptime("2025-06-01 00:00:00", "%Y-%m-%d %H:%M:%S")
end = datetime.datetime.strptime("2025-06-28 00:00:00", "%Y-%m-%d %H:%M:%S")
btime1 = datetime.datetime(
                2022, 9, 5
            )
etime1 = datetime.datetime(
                2022, 9, 15
            )

def read4(path):
    events = {}
    prov = []
    with open(path, "r") as f:
        lines = f.readlines()
        for line in lines:
            #642358818432356352,2025/3/1 0:06:03.839,99.9851,25.1268,11.0,云南省大理白族自治州巍山彝族回族自治
            parts = line.split()

            year = int(parts[3])
            month = int(parts[4])
            day = int(parts[5])
            hour = int(parts[7])
            minute = int(parts[8])
            second = int(parts[9])
            microsecond = int(parts[10])

            # 构造发震时刻（UTC/本地你可以自己再转换）
            etime = datetime.datetime(
                year, month, day,
                hour, minute, second,
                microsecond
            )
            if etime < btime1 or etime > etime1:
                continue
            try:
                i_loc = parts.index("LOC")
                i_dep = parts.index("DEP")
                i_mag = parts.index("MAG")
            except ValueError:
                # 如果某一项缺失，可以选择返回 None 或者抛异常
                return None
            try:
                lon = float(parts[i_loc + 1])
                lat = float(parts[i_loc + 2])
                depth_km = float(parts[i_dep + 1])
                mag_type = parts[i_mag + 1]
                mag = float(parts[i_mag + 2])
            except:
                continue 
            #if etime < begin or etime > end:
            #    continue
            delta = (etime-btime).total_seconds()
            tkey = int(delta)
            eloc = [lon, lat, depth_km, delta]
            events[tkey] = eloc
    #print(set(prov)) 
    return events


def gelman_rubin(chains):
    """
    chains: numpy array, shape = (m, n)
        m = number of chains
        n = length of each chain after burn-in
    """
    m, n = chains.shape

    # 每条链的均值
    chain_means = np.mean(chains, axis=1)

    # 整体均值
    overall_mean = np.mean(chain_means)

    # B: 链间方差
    B = n * np.sum((chain_means - overall_mean)**2) / (m - 1)

    # W: 链内方差
    W = np.mean(np.var(chains, axis=1, ddof=1))

    # V_hat: 后验方差估计
    V_hat = (1 - 1/(n+1e-6)) * W + (B / (n+1e-6))

    # R_hat: Gelman-Rubin statistic
    R_hat = np.sqrt(V_hat / W)
    return R_hat



# ---------------------------
# 示例：生成数据并运行（含缺测示例）
# ---------------------------
import random 
import tqdm 
import numpy as np
import matplotlib.pyplot as plt
import matplotlib.gridspec as gridspec 

import matplotlib as mpl

mpl.rcParams.update({
    "font.size": 8,
    "axes.labelsize": 8,
    "axes.titlesize": 8,
    "xtick.labelsize": 7,
    "ytick.labelsize": 7,
    "legend.fontsize": 7,
    "axes.linewidth": 0.6,
    "xtick.major.width": 0.6,
    "ytick.major.width": 0.6,
    "xtick.minor.width": 0.5,
    "ytick.minor.width": 0.5,
    "xtick.direction": "out",
    "ytick.direction": "out",
    "pdf.fonttype": 42,   # TrueType, JGR/AGU 常用
    "ps.fonttype": 42,
})

# -*- coding: utf-8 -*-
import numpy as np
import matplotlib.pyplot as plt
import matplotlib as mpl
import matplotlib.gridspec as gridspec

# =========================
# JGR-ish style (print friendly)
# =========================
mpl.rcParams.update({
    "font.size": 8,
    "axes.labelsize": 8,
    "axes.titlesize": 8,
    "xtick.labelsize": 7,
    "ytick.labelsize": 7,
    "legend.fontsize": 7,
    "axes.linewidth": 0.6,
    "xtick.major.width": 0.6,
    "ytick.major.width": 0.6,
    "xtick.minor.width": 0.5,
    "ytick.minor.width": 0.5,
    "xtick.direction": "out",
    "ytick.direction": "out",
    "pdf.fonttype": 42,   # TrueType
    "ps.fonttype": 42,
})


def add_panel_label(ax, label):
    ax.text(0.02, 1.01, label, transform=ax.transAxes,
            ha="left", va="bottom", fontsize=8, fontweight="bold")


def add_rhat_box(ax, text, loc=(0.98, 1.01)):
    ax.text(loc[0], loc[1], text,
            transform=ax.transAxes, ha="right", va="bottom",
            fontsize=7,
            bbox=dict(boxstyle="round,pad=0.18", fc="white", ec="0.5", lw=0.5, alpha=0.92))


def nice_axes(ax):
    ax.grid(True, which="major", lw=0.35, alpha=0.25)
    ax.tick_params(length=3.0, width=0.6)
    ax.tick_params(which="minor", length=2.0, width=0.5)


# =========================
# YOU MUST HAVE gelman_rubin(values) defined elsewhere
# values shape expected: (nchain, nsamp, ndim)
# =========================
# def gelman_rubin(values): ...

# -*- coding: utf-8 -*-
import numpy as np
import matplotlib.pyplot as plt
import matplotlib as mpl
import matplotlib.gridspec as gridspec

mpl.rcParams.update({
    "font.size": 8,
    "axes.labelsize": 8,
    "axes.titlesize": 8,
    "xtick.labelsize": 7,
    "ytick.labelsize": 7,
    "legend.fontsize": 7,
    "axes.linewidth": 0.6,
    "xtick.major.width": 0.6,
    "ytick.major.width": 0.6,
    "xtick.minor.width": 0.5,
    "ytick.minor.width": 0.5,
    "xtick.direction": "out",
    "ytick.direction": "out",
    "pdf.fonttype": 42,
    "ps.fonttype": 42,
})


def add_panel_label(ax, label):
    ax.text(0.02, 1.01, label, transform=ax.transAxes,
            ha="left", va="bottom", fontsize=8, fontweight="bold")


def add_rhat_box(ax, text, loc=(0.98, 1.01)):
    ax.text(loc[0], loc[1], text,
            transform=ax.transAxes, ha="right", va="bottom",
            fontsize=7,
            bbox=dict(boxstyle="round,pad=0.18", fc="white", ec="0.5", lw=0.5, alpha=0.92))


def nice_axes(ax):
    ax.grid(True, which="major", lw=0.35, alpha=0.25)
    ax.tick_params(length=3.0, width=0.6)
    ax.tick_params(which="minor", length=2.0, width=0.5)


def _to_numpy(x):
    try:
        import torch
        if isinstance(x, torch.Tensor):
            return x.detach().cpu().numpy()
    except Exception:
        pass
    return np.asarray(x)


def _pad_lim(vmin, vmax, pad):
    """pad can be: float (absolute) or tuple/list (left,right)."""
    if isinstance(pad, (tuple, list, np.ndarray)) and len(pad) == 2:
        pl, pr = float(pad[0]), float(pad[1])
    else:
        pl = pr = float(pad)
    return vmin - pl, vmax + pr


# =========================
# YOU MUST HAVE gelman_rubin(values) defined elsewhere
# values shape expected: (nchain, nsamp, ndim)
# =========================
# def gelman_rubin(values): ...


def plot_sample_path_jgr(
    xs_samples,
    t0_samples,
    xs_init,
    start=500,
    out_png="figures/figs/sample_path.png",
    out_pdf="figures/figs/sample_path.pdf",
    # ===== NEW: fixed pads for axis limits (absolute units) =====
    pad_x_km=2.0,
    pad_y_km=2.0,
    pad_z_km=1.0,
):
    """
    JGR-style 2x4 panels:
      Top row: X-Y
      Bottom row: X-Depth(Z)

    Axis ranges are GLOBAL across all panels:
      [min(all samples), max(all samples)] + fixed pad.

    Parameters
    ----------
    xs_samples: torch.Tensor or np.ndarray, shape [Nsamp, 16, 3]
    t0_samples: torch.Tensor or np.ndarray, shape [Nsamp, 16]
    xs_init:    np.ndarray, shape [16, 3]
    start: burn-in cutoff index
    pad_x_km/pad_y_km/pad_z_km: absolute pads added to global min/max (km)
    """

    xs_samples_np = _to_numpy(xs_samples)   # [Nsamp, 16, 3]
    t0_samples_np = _to_numpy(t0_samples)   # [Nsamp, 16]
    xs_init_np = _to_numpy(xs_init)         # [16, 3]

    # =========================
    # 1) GLOBAL limits from ALL sampling points (after burn-in) + fixed pad
    #    Use start::2 to match the points you actually plot as "Samples"
    # =========================
    thin = slice(start, None, 2)
    xs_thin = xs_samples_np[thin, :, :]  # [Ns_eff, 16, 3]

    x_all = xs_thin[:, :, 0].ravel()
    y_all = xs_thin[:, :, 1].ravel()
    z_all = xs_thin[:, :, 2].ravel()

    xlim = _pad_lim(np.nanmin(x_all), np.nanmax(x_all), pad_x_km)
    ylim = _pad_lim(np.nanmin(y_all), np.nanmax(y_all), pad_y_km)
    zlim = _pad_lim(np.nanmin(z_all), np.nanmax(z_all), pad_z_km)

    # =========================
    # 2) Figure + axes
    # =========================
    fig = plt.figure(figsize=(12, 6.2), dpi=300)
    gs = gridspec.GridSpec(
        2, 4,
        left=0.08,
        right=0.98,
        top=0.95,
        bottom=0.25,
        wspace=0.35,
        hspace=0.28
    )
    axs = [plt.subplot(gs[0, i]) for i in range(4)]
    axs2 = [plt.subplot(gs[1, i]) for i in range(4)]

    colors = ["#0072B2", "#D55E00", "#009E73", "#CC79A7"]  # Okabe–Ito
    burn_color = "0.25"

    rhat_xy = [None] * 4
    rhat_zt = [None] * 4

    # =========================
    # 3) Plot and compute Rhat per idx
    # =========================
    for idx in range(4):
        ax = axs[idx]
        ax2 = axs2[idx]
        values = []

        for c in range(4):
            j = idx + c * 4

            xs_samps = xs_samples_np[:, j, :]  # [Nsamp,3]
            t0_samps = t0_samples_np[:, j]     # [Nsamp,]

            xs_mean = xs_samps.mean(axis=0)
            x1, y1, z1 = xs_mean[0], xs_mean[1], xs_mean[2]

            x0, y0, z0 = xs_init_np[j, 0], xs_init_np[j, 1], xs_init_np[j, 2]

            eff = np.stack([xs_samps[thin, 0],
                            xs_samps[thin, 1],
                            xs_samps[thin, 2],
                            t0_samps[thin]], axis=1)
            values.append(eff)

            xs_path = np.concatenate([np.array([[x0, y0, z0]]), xs_samps], axis=0)

            # ---- XY
            ax.plot(xs_path[:start, 0], xs_path[:start, 1],
                    '-', color=burn_color, lw=0.35, alpha=0.25,
                    label="Burn-in" if c == 0 else None, zorder=1)

            ax.plot(xs_path[start::2, 0], xs_path[start::2, 1],
                    '-', color=colors[c], lw=0.35, alpha=0.18,
                    label="Sample path" if c == 0 else None, zorder=2)

            ax.scatter(xs_path[start::2, 0], xs_path[start::2, 1],
                       color=colors[c], s=1.0, alpha=0.12, linewidths=0,
                       label="Samples" if c == 0 else None, zorder=2)

            ax.scatter([x1], [y1], marker='+', s=55, c="k", linewidths=1.0,
                       label="Posterior mean" if c == 0 else None, zorder=4)

            ax.scatter([x0], [y0], marker='^', s=40, facecolors="none",
                       edgecolors="k", linewidths=0.9,
                       label="Start point" if c == 0 else None, zorder=4)

            # ---- XZ
            ax2.plot(xs_path[:start, 0], xs_path[:start, 2],
                     '-', color=burn_color, lw=0.35, alpha=0.25,
                     label="Burn-in" if c == 0 else None, zorder=1)

            ax2.plot(xs_path[start::2, 0], xs_path[start::2, 2],
                     '-', color=colors[c], lw=0.35, alpha=0.18,
                     label="Sample path" if c == 0 else None, zorder=2)

            ax2.scatter(xs_path[start::2, 0], xs_path[start::2, 2],
                        color=colors[c], s=1.0, alpha=0.12, linewidths=0,
                        label="Samples" if c == 0 else None, zorder=2)

            ax2.scatter([x1], [z1], marker='+', s=55, c="k", linewidths=1.0,
                        label="Posterior mean" if c == 0 else None, zorder=4)

            ax2.scatter([x0], [z0], marker='^', s=40, facecolors="none",
                        edgecolors="k", linewidths=0.9,
                        label="Start point" if c == 0 else None, zorder=4)

        # ---- Rhat
        values = np.stack(values, axis=0)  # [4, Ns_eff, 4]
        rhats = [float(gelman_rubin(values[:, :, k])) for k in range(4)]
        rhat_xy[idx] = (rhats[0], rhats[1])
        rhat_zt[idx] = (rhats[2], rhats[3])

    # =========================
    # 4) Labels + Rhat + GLOBAL limits (same for all panels)
    # =========================
    labels_top = ['(a)', '(b)', '(c)', '(d)']
    labels_bot = ['(e)', '(f)', '(g)', '(h)']

    for i, ax in enumerate(axs):
        ax.set_xlabel("X (km)")
        ax.set_ylabel("Y (km)")
        add_panel_label(ax, labels_top[i])

        rx, ry = rhat_xy[i]
        add_rhat_box(ax, rf"$\hat{{R}}_{{x}}$={rx:.3f}  $\hat{{R}}_{{y}}$={ry:.3f}")

        nice_axes(ax)
        ax.set_xlim(*xlim)
        ax.set_ylim(*ylim)

    for i, ax in enumerate(axs2):
        ax.set_xlabel("X (km)")
        ax.set_ylabel("Depth (km)")
        ax.invert_yaxis()
        add_panel_label(ax, labels_bot[i])

        rz, rt = rhat_zt[i]
        add_rhat_box(ax, rf"$\hat{{R}}_{{z}}$={rz:.3f}  $\hat{{R}}_{{t_0}}$={rt:.3f}")

        nice_axes(ax)
        ax.set_xlim(*xlim)
        ax.set_ylim(*zlim)

    # =========================
    # 5) Global legend
    # =========================
    handles, leglabels = axs[0].get_legend_handles_labels()
    for ax in axs + axs2:
        leg = ax.get_legend()
        if leg is not None:
            leg.remove()

    fig.legend(handles, leglabels,
               loc='upper center',
               bbox_to_anchor=(0.5, 0.18),
               ncol=5,
               frameon=True,
               fancybox=True,
               borderpad=0.3,
               handlelength=1.6,
               handletextpad=0.6,
               columnspacing=1.0)

    leg = fig.legends[0]
    leg.get_frame().set_facecolor("white")
    leg.get_frame().set_edgecolor("0.6")
    leg.get_frame().set_linewidth(0.6)
    leg.get_frame().set_alpha(0.95)

    # =========================
    # 6) Save
    # =========================
    fig.savefig(out_png, dpi=300)
    fig.savefig(out_pdf, dpi=300)
    plt.close(fig)

# -*- coding: utf-8 -*-
import numpy as np
import matplotlib.pyplot as plt
import matplotlib as mpl
import matplotlib.gridspec as gridspec

mpl.rcParams.update({
    "font.size": 8,
    "axes.labelsize": 8,
    "axes.titlesize": 8,
    "xtick.labelsize": 7,
    "ytick.labelsize": 7,
    "legend.fontsize": 7,
    "axes.linewidth": 0.6,
    "xtick.major.width": 0.6,
    "ytick.major.width": 0.6,
    "xtick.minor.width": 0.5,
    "ytick.minor.width": 0.5,
    "xtick.direction": "out",
    "ytick.direction": "out",
    "pdf.fonttype": 42,
    "ps.fonttype": 42,
})


def add_panel_label(ax, label):
    ax.text(0.02, 1.01, label, transform=ax.transAxes,
            ha="left", va="bottom", fontsize=8, fontweight="bold")


def add_rhat_box(ax, text, loc=(0.98, 1.01)):
    ax.text(loc[0], loc[1], text,
            transform=ax.transAxes, ha="right", va="bottom",
            fontsize=7,
            bbox=dict(boxstyle="round,pad=0.18", fc="white", ec="0.5", lw=0.5, alpha=0.92))


def nice_axes(ax):
    ax.grid(True, which="major", lw=0.35, alpha=0.25)
    ax.tick_params(length=3.0, width=0.6)
    ax.tick_params(which="minor", length=2.0, width=0.5)


def _to_numpy(x):
    try:
        import torch
        if isinstance(x, torch.Tensor):
            return x.detach().cpu().numpy()
    except Exception:
        pass
    return np.asarray(x)


def _pad_lim(vmin, vmax, pad):
    pad = float(pad)
    return vmin - pad, vmax + pad


# =========================
# YOU MUST HAVE gelman_rubin(values) defined elsewhere
# values shape expected: (nchain, nsamp, ndim)
# =========================
# def gelman_rubin(values): ...


def plot_sample_path_jgr_sharex_pairs(
    xs_samples,
    t0_samples,
    xs_init,
    start=500,
    out_png="figures/figs/sample_path.png",
    out_pdf="figures/figs/sample_path.pdf",
    # fixed absolute pads (km) for per-panel limits
    pad_x_km=2.0,
    pad_y_km=2.0,
    pad_z_km=1.0,
):
    """
    Required behavior:
      - Share X-limits ONLY for vertical pairs: (a,e), (b,f), (c,g), (d,h)
      - Other axes (Y for top row, Z/depth for bottom row) are independent per subplot
      - X-limits are computed per pair from *all sampling points* in that pair + fixed pad
      - Each subplot defines its own Y/Z limits from its own sampling points + fixed pad
    """

    xs_samples_np = _to_numpy(xs_samples)   # [Nsamp, 16, 3]
    t0_samples_np = _to_numpy(t0_samples)   # [Nsamp, 16]
    xs_init_np = _to_numpy(xs_init)         # [16, 3]

    thin = slice(start, None, 2)
    thin_lim = slice(0, None, 1)

    # -------------------------
    # Precompute per-panel ranges:
    #   xlims_pair[idx] shared by (idx in top) and (idx in bottom)
    #   ylims_top[idx]  for (a-d) individually
    #   zlims_bot[idx]  for (e-h) individually
    # -------------------------
    xlims_pair = [None] * 4
    ylims_top  = [None] * 4
    zlims_bot  = [None] * 4
    xlims_pair_lim = [None] * 4
    ylims_top_lim  = [None] * 4
    zlims_bot_lim  = [None] * 4
    for idx in range(4):
        # collect all 4 chains for this idx for top/bottom to share X
        xs_pair = []  # (x samples only) from both the XY and XZ views -> same x anyway

        ys_top = []   # y samples for top (XY)
        zs_bot = []   # z samples for bottom (XZ)
        
        xs_pair_lim = []  # (x samples only) from both the XY and XZ views -> same x anyway
        ys_top_lim = []   # y samples for top (XY)
        zs_bot_lim = []   # z samples for bottom (XZ)
        for c in range(4):
            j = idx + c * 4
            xs = xs_samples_np[thin, j, :]  # [Ns_eff,3]
            xs_pair.append(xs[:, 0])        # x
            ys_top.append(xs[:, 1])         # y
            zs_bot.append(xs[:, 2])         # z
            xs_pair_lim.append(xs_samples_np[thin_lim, j, :])
            ys_top_lim.append(xs_samples_np[thin_lim, j, 1])
            zs_bot_lim.append(xs_samples_np[thin_lim, j, 2])


        x_all = np.concatenate(xs_pair, axis=0)
        y_all = np.concatenate(ys_top, axis=0)
        z_all = np.concatenate(zs_bot, axis=0)

        x_all_lim = np.concatenate(xs_pair_lim, axis=0) 
        y_all_lim = np.concatenate(ys_top_lim, axis=0)
        z_all_lim = np.concatenate(zs_bot_lim, axis=0)

        xlims_pair[idx] = _pad_lim(np.nanmin(x_all_lim), np.nanmax(x_all_lim), pad_x_km)
        ylims_top[idx]  = _pad_lim(np.nanmin(y_all_lim), np.nanmax(y_all_lim), pad_y_km)
        zlims_bot[idx]  = _pad_lim(np.nanmin(z_all_lim), np.nanmax(z_all_lim), pad_z_km)

    # -------------------------
    # Figure layout: sharex within each vertical pair
    # -------------------------
    fig = plt.figure(figsize=(12, 6.2), dpi=300)
    gs = gridspec.GridSpec(
        2, 4,
        left=0.08,
        right=0.98,
        top=0.95,
        bottom=0.25,
        wspace=0.35,
        hspace=0.28
    )

    axs  = [plt.subplot(gs[0, i]) for i in range(4)]            # a-d
    axs2 = [plt.subplot(gs[1, i], sharex=axs[i]) for i in range(4)]  # e-h share x with a-d by column

    colors = ["#0072B2", "#D55E00", "#009E73", "#CC79A7"]  # Okabe–Ito
    burn_color = "0.25"

    rhat_xy = [None] * 4
    rhat_zt = [None] * 4

    # -------------------------
    # Plot
    # -------------------------
    for idx in range(4):
        ax = axs[idx]
        ax2 = axs2[idx]
        values = []

        for c in range(4):
            j = idx + c * 4
            xs_samps = xs_samples_np[:, j, :]  # [Nsamp,3]
            t0_samps = t0_samples_np[:, j]     # [Nsamp,]

            xs_mean = xs_samps.mean(axis=0)
            x1, y1, z1 = xs_mean[0], xs_mean[1], xs_mean[2]

            x0, y0, z0 = xs_init_np[j, 0], xs_init_np[j, 1], xs_init_np[j, 2]

            eff = np.stack([xs_samps[thin, 0],
                            xs_samps[thin, 1],
                            xs_samps[thin, 2],
                            t0_samps[thin]], axis=1)
            values.append(eff)

            xs_path = np.concatenate([np.array([[x0, y0, z0]]), xs_samps], axis=0)

            # ---- XY
            ax.plot(xs_path[:start, 0], xs_path[:start, 1],
                    '-', color=burn_color, lw=0.35, alpha=0.25,
                    label="Burn-in" if c == 0 else None, zorder=1)

            ax.plot(xs_path[start::2, 0], xs_path[start::2, 1],
                    '-', color=colors[c], lw=0.35, alpha=0.18,
                    label="Sample path" if c == 0 else None, zorder=2)

            ax.scatter(xs_path[start::2, 0], xs_path[start::2, 1],
                       color=colors[c], s=1.0, alpha=0.12, linewidths=0,
                       label="Samples" if c == 0 else None, zorder=2)

            ax.scatter([x1], [y1], marker='+', s=55, c="k", linewidths=1.0,
                       label="Posterior mean" if c == 0 else None, zorder=4)

            ax.scatter([x0], [y0], marker='^', s=40, facecolors="none",
                       edgecolors="k", linewidths=0.9,
                       label="Start point" if c == 0 else None, zorder=4)

            # ---- XZ
            ax2.plot(xs_path[:start, 0], xs_path[:start, 2],
                     '-', color=burn_color, lw=0.35, alpha=0.25,
                     label="Burn-in" if c == 0 else None, zorder=1)

            ax2.plot(xs_path[start::2, 0], xs_path[start::2, 2],
                     '-', color=colors[c], lw=0.35, alpha=0.18,
                     label="Sample path" if c == 0 else None, zorder=2)

            ax2.scatter(xs_path[start::2, 0], xs_path[start::2, 2],
                        color=colors[c], s=1.0, alpha=0.12, linewidths=0,
                        label="Samples" if c == 0 else None, zorder=2)

            ax2.scatter([x1], [z1], marker='+', s=55, c="k", linewidths=1.0,
                        label="Posterior mean" if c == 0 else None, zorder=4)

            ax2.scatter([x0], [z0], marker='^', s=40, facecolors="none",
                        edgecolors="k", linewidths=0.9,
                        label="Start point" if c == 0 else None, zorder=4)

        # Rhat per idx
        values = np.stack(values, axis=0)  # [4, Ns_eff, 4]
        rhats = [float(gelman_rubin(values[:, :, k])) for k in range(4)]
        rhat_xy[idx] = (rhats[0], rhats[1])  # X,Y
        rhat_zt[idx] = (rhats[2], rhats[3])  # Z,t0

    # -------------------------
    # Labels + Rhat + limits
    #   - shared x per column (a,e) etc via sharex
    #   - still set xlim explicitly per pair for determinism
    #   - y/z individual per subplot
    # -------------------------
    labels_top = ['(a)', '(b)', '(c)', '(d)']
    labels_bot = ['(e)', '(f)', '(g)', '(h)']

    for i, ax in enumerate(axs):
        ax.set_xlabel("X (km)")
        ax.set_ylabel("Y (km)")
        add_panel_label(ax, labels_top[i])

        rx, ry = rhat_xy[i]
        add_rhat_box(ax, rf"$\hat{{R}}_{{x}}$={rx:.3f}  $\hat{{R}}_{{y}}$={ry:.3f}")

        nice_axes(ax)
        ax.set_xlim(*xlims_pair[i])     # shared with bottom via sharex
        ax.set_ylim(*ylims_top[i])      # independent per top panel

        # Optional: hide top x tick labels to reduce clutter (common in JGR)
        ax.tick_params(labelbottom=False)

    for i, ax in enumerate(axs2):
        ax.set_xlabel("X (km)")
        ax.set_ylabel("Depth (km)")
        ax.invert_yaxis()
        add_panel_label(ax, labels_bot[i])

        rz, rt = rhat_zt[i]
        add_rhat_box(ax, rf"$\hat{{R}}_{{z}}$={rz:.3f}  $\hat{{R}}_{{t_0}}$={rt:.3f}")

        nice_axes(ax)
        ax.set_xlim(*xlims_pair[i])     # shared with top
        ax.set_ylim(*zlims_bot[i])      # independent per bottom panel

    # -------------------------
    # Global legend
    # -------------------------
    handles, leglabels = axs[0].get_legend_handles_labels()
    for ax in axs + axs2:
        leg = ax.get_legend()
        if leg is not None:
            leg.remove()

    fig.legend(handles, leglabels,
               loc='upper center',
               bbox_to_anchor=(0.5, 0.18),
               ncol=5,
               frameon=True,
               fancybox=True,
               borderpad=0.3,
               handlelength=1.6,
               handletextpad=0.6,
               columnspacing=1.0)

    leg = fig.legends[0]
    leg.get_frame().set_facecolor("white")
    leg.get_frame().set_edgecolor("0.6")
    leg.get_frame().set_linewidth(0.6)
    leg.get_frame().set_alpha(0.95)

    fig.savefig(out_png, dpi=300)
    fig.savefig(out_pdf, dpi=300)
    plt.close(fig)

# -*- coding: utf-8 -*-
import numpy as np
import matplotlib.pyplot as plt
import matplotlib as mpl
import matplotlib.gridspec as gridspec

mpl.rcParams.update({
    "font.size": 8,
    "axes.labelsize": 8,
    "axes.titlesize": 8,
    "xtick.labelsize": 7,
    "ytick.labelsize": 7,
    "legend.fontsize": 7,
    "axes.linewidth": 0.6,
    "xtick.major.width": 0.6,
    "ytick.major.width": 0.6,
    "xtick.minor.width": 0.5,
    "ytick.minor.width": 0.5,
    "xtick.direction": "out",
    "ytick.direction": "out",
    "pdf.fonttype": 42,
    "ps.fonttype": 42,
})


def add_panel_label(ax, label):
    ax.text(0.02, 1.01, label, transform=ax.transAxes,
            ha="left", va="bottom", fontsize=8, fontweight="bold")


def add_rhat_box(ax, text, loc=(0.98, 1.01)):
    ax.text(loc[0], loc[1], text,
            transform=ax.transAxes, ha="right", va="bottom",
            fontsize=7,
            bbox=dict(boxstyle="round,pad=0.18", fc="white", ec="0.5", lw=0.5, alpha=0.92))


def nice_axes(ax):
    ax.grid(True, which="major", lw=0.35, alpha=0.25)
    ax.tick_params(length=3.0, width=0.6)
    ax.tick_params(which="minor", length=2.0, width=0.5)


def _to_numpy(x):
    try:
        import torch
        if isinstance(x, torch.Tensor):
            return x.detach().cpu().numpy()
    except Exception:
        pass
    return np.asarray(x)


def _pad_lim(vmin, vmax, pad):
    pad = float(pad)
    if not np.isfinite(vmin) or not np.isfinite(vmax):
        return (-1.0, 1.0)
    if vmin == vmax:
        return (vmin - pad, vmax + pad)
    return (vmin - pad, vmax + pad)


# =========================
# YOU MUST HAVE gelman_rubin(values) defined elsewhere
# values shape expected: (nchain, nsamp, ndim)
# =========================
# def gelman_rubin(values): ...


def plot_sample_path_jgr_sharex_pairs_v2(
    xs_samples,
    t0_samples,
    xs_init,
    start=500,
    out_png="figures/figs/sample_path.v1.0.png",
    out_pdf="figures/figs/sample_path.v1.0.pdf",
    # fixed absolute pads (km) for axis limits
    pad_x_km=10.0,
    pad_y_km=10.0,
    pad_z_km=10.0,
    # if your z is NOT "depth-positive-down", set depth_from_z = lambda z: -z
    depth_from_z=None,
):
    """
    Behavior:
      - Share X-limits only for vertical pairs: (a,e), (b,f), (c,g), (d,h)
      - Each panel has its own Y/Depth limits (no cross-panel max/min sharing)
      - Axis limits include: ALL samples (full chain, including burn-in) + start point + fixed pad
      - thin is used ONLY for R-hat computation (start::2)
      - Depth displayed positive downward (invert_yaxis)
    """

    xs_samples_np = _to_numpy(xs_samples)   # [Nsamp, 16, 3]
    t0_samples_np = _to_numpy(t0_samples)   # [Nsamp, 16]
    xs_init_np = _to_numpy(xs_init)         # [16, 3]

    if depth_from_z is None:
        # assume z already represents depth (positive downward)
        depth_from_z = lambda z: z

    # thin only for Rhat
    thin = slice(start, None, 2)

    # -------------------------
    # Precompute per-column (pair) X limits and per-panel Y/Depth limits
    # IMPORTANT: DO NOT use thin here; include all samples + start point
    # -------------------------
    xlims_pair = [None] * 4
    ylims_top  = [None] * 4
    dlims_bot  = [None] * 4  # depth limits for bottom row

    for idx in range(4):
        x_all = []
        y_all = []
        d_all = []

        for c in range(4):
            j = idx + c * 4
            xs_full = xs_samples_np[:, j, :]  # full trajectory [Nsamp,3]

            x_all.append(xs_full[:, 0])
            y_all.append(xs_full[:, 1])
            d_all.append(depth_from_z(xs_full[:, 2]))

            # include start point explicitly
            x0, y0, z0 = xs_init_np[j, 0], xs_init_np[j, 1], xs_init_np[j, 2]
            x_all.append(np.array([x0]))
            y_all.append(np.array([y0]))
            d_all.append(np.array([depth_from_z(z0)]))

        x_all = np.concatenate(x_all, axis=0)
        y_all = np.concatenate(y_all, axis=0)
        d_all = np.concatenate(d_all, axis=0)

        xlims_pair[idx] = _pad_lim(np.nanmin(x_all), np.nanmax(x_all), pad_x_km)
        ylims_top[idx]  = _pad_lim(np.nanmin(y_all), np.nanmax(y_all), pad_y_km)
        dlims_bot[idx]  = _pad_lim(np.nanmin(d_all), np.nanmax(d_all), pad_z_km)

    # -------------------------
    # Figure layout + sharex by column
    # -------------------------
    fig = plt.figure(figsize=(12, 6.2), dpi=300)
    gs = gridspec.GridSpec(
        2, 4,
        left=0.08,
        right=0.98,
        top=0.95,
        bottom=0.25,
        wspace=0.35,
        hspace=0.28
    )

    axs  = [plt.subplot(gs[0, i]) for i in range(4)]              # (a)-(d)
    axs2 = [plt.subplot(gs[1, i], sharex=axs[i]) for i in range(4)]  # (e)-(h)

    colors = ["#0072B2", "#D55E00", "#009E73", "#CC79A7"]  # Okabe–Ito
    burn_color = "0.25"

    rhat_xy = [None] * 4
    rhat_dt = [None] * 4  # (depth,t0) RHat shown on bottom row

    # -------------------------
    # Plot
    # -------------------------
    for idx in range(4):
        ax = axs[idx]
        ax2 = axs2[idx]
        values = []

        for c in range(4):
            j = idx + c * 4
            xs_samps = xs_samples_np[:, j, :]  # [Nsamp,3]
            t0_samps = t0_samples_np[:, j]     # [Nsamp,]

            # mean (full chain mean for display)
            xs_mean = xs_samps.mean(axis=0)
            x1, y1, z1 = xs_mean[0], xs_mean[1], xs_mean[2]
            d1 = depth_from_z(z1)

            # init
            x0, y0, z0 = xs_init_np[j, 0], xs_init_np[j, 1], xs_init_np[j, 2]
            d0 = depth_from_z(z0)

            # Rhat uses thinned samples after burn-in
            eff = np.stack([xs_samps[thin, 0],
                            xs_samps[thin, 1],
                            depth_from_z(xs_samps[thin, 2]),
                            t0_samps[thin]], axis=1)
            values.append(eff)

            # add init point for path plotting
            xs_path = np.concatenate([np.array([[x0, y0, z0]]), xs_samps], axis=0)
            d_path = depth_from_z(xs_path[:, 2])

            # ---- XY (top)
            ax.plot(xs_path[:start, 0], xs_path[:start, 1],
                    '-', color=burn_color, lw=0.35, alpha=0.25,
                    label="Burn-in" if c == 0 else None, zorder=1)

            ax.plot(xs_path[start::2, 0], xs_path[start::2, 1],
                    '-', color=colors[c], lw=0.35, alpha=0.18,
                    label="Sample path" if c == 0 else None, zorder=2)

            ax.scatter(xs_path[start::2, 0], xs_path[start::2, 1],
                       color=colors[c], s=1.0, alpha=0.12, linewidths=0,
                       label="Samples" if c == 0 else None, zorder=2)

            ax.scatter([x1], [y1], marker='+', s=55, c="k", linewidths=1.0,
                       label="Posterior mean" if c == 0 else None, zorder=4)

            ax.scatter([x0], [y0], marker='^', s=40, facecolors="none",
                       edgecolors="k", linewidths=0.9,
                       label="Start point" if c == 0 else None, zorder=4)

            # ---- X-Depth (bottom)
            ax2.plot(xs_path[:start, 0], d_path[:start],
                     '-', color=burn_color, lw=0.35, alpha=0.25,
                     label="Burn-in" if c == 0 else None, zorder=1)

            ax2.plot(xs_path[start::2, 0], d_path[start::2],
                     '-', color=colors[c], lw=0.35, alpha=0.18,
                     label="Sample path" if c == 0 else None, zorder=2)

            ax2.scatter(xs_path[start::2, 0], d_path[start::2],
                        color=colors[c], s=1.0, alpha=0.12, linewidths=0,
                        label="Samples" if c == 0 else None, zorder=2)

            ax2.scatter([x1], [d1], marker='+', s=55, c="k", linewidths=1.0,
                        label="Posterior mean" if c == 0 else None, zorder=4)

            ax2.scatter([x0], [d0], marker='^', s=40, facecolors="none",
                        edgecolors="k", linewidths=0.9,
                        label="Start point" if c == 0 else None, zorder=4)

        # ---- Rhat per idx (thin only)
        values = np.stack(values, axis=0)  # [4 chains, Ns_eff, 4]
        rx = float(gelman_rubin(values[:, :, 0]))
        ry = float(gelman_rubin(values[:, :, 1]))
        rd = float(gelman_rubin(values[:, :, 2]))  # depth rhat
        rt = float(gelman_rubin(values[:, :, 3]))  # t0 rhat

        rhat_xy[idx] = (rx, ry)
        rhat_dt[idx] = (rd, rt)

    # -------------------------
    # Labels + Rhat + limits
    # -------------------------
    labels_top = ['(a)', '(b)', '(c)', '(d)']
    labels_bot = ['(e)', '(f)', '(g)', '(h)']

    for i, ax in enumerate(axs):
        ax.set_xlabel("X (km)")
        ax.set_ylabel("Y (km)")
        add_panel_label(ax, labels_top[i])

        rx, ry = rhat_xy[i]
        add_rhat_box(ax, rf"$\hat{{R}}_x$={rx:.3f}  $\hat{{R}}_y$={ry:.3f}")

        nice_axes(ax)
        ax.set_xlim(*xlims_pair[i])    # shared with bottom via sharex
        ax.set_ylim(*ylims_top[i])     # per-panel
        ax.tick_params(labelbottom=False)

    for i, ax in enumerate(axs2):
        ax.set_xlabel("X (km)")
        ax.set_ylabel("Depth (km)")
        add_panel_label(ax, labels_bot[i])

        rd, rt = rhat_dt[i]
        add_rhat_box(ax, rf"$\hat{{R}}_z$={rd:.3f}  $\hat{{R}}_{{t_0}}$={rt:.3f}")

        nice_axes(ax)
        ax.set_xlim(*xlims_pair[i])
        ax.set_ylim(*dlims_bot[i])

        # depth positive downward in display
        ax.invert_yaxis()

    # -------------------------
    # Global legend
    # -------------------------
    handles, leglabels = axs[0].get_legend_handles_labels()
    for ax in axs + axs2:
        leg = ax.get_legend()
        if leg is not None:
            leg.remove()

    fig.legend(handles, leglabels,
               loc='upper center',
               bbox_to_anchor=(0.5, 0.18),
               ncol=5,
               frameon=True,
               fancybox=True,
               borderpad=0.3,
               handlelength=1.6,
               handletextpad=0.6,
               columnspacing=1.0)

    leg = fig.legends[0]
    leg.get_frame().set_facecolor("white")
    leg.get_frame().set_edgecolor("0.6")
    leg.get_frame().set_linewidth(0.6)
    leg.get_frame().set_alpha(0.95)

    fig.savefig(out_png, dpi=300)
    fig.savefig(out_pdf, dpi=300)
    plt.close(fig)


# =========================
# Usage
# =========================
# 1) If your z is already depth-positive-down:
# plot_sample_path_jgr_sharex_pairs(xs_samples, t0_samples, xs_init, start=500,
#                                  pad_x_km=2.0, pad_y_km=2.0, pad_z_km=2.0)

# 2) If your z is elevation-positive-up and you want depth-positive-down:
# plot_sample_path_jgr_sharex_pairs(xs_samples, t0_samples, xs_init, start=500,
#                                  pad_x_km=2.0, pad_y_km=2.0, pad_z_km=2.0,
#                                  depth_from_z=lambda z: -z)

# =========================
# Usage
# =========================
# plot_sample_path_jgr_sharex_pairs(xs_samples=xs_samples, t0_samples=t0_samples, xs_init=xs_init,
#                                  start=500, pad_x_km=2.0, pad_y_km=2.0, pad_z_km=1.0)

# =========================
# Usage
# =========================
# plot_sample_path_jgr(xs_samples=xs_samples, t0_samples=t0_samples, xs_init=xs_init,
#                      start=500, pad_x_km=2.0, pad_y_km=2.0, pad_z_km=1.0)



# =========================
# Usage (replace your block with this call)
# =========================
# plot_sample_path_jgr(xs_samples=xs_samples, t0_samples=t0_samples, xs_init=xs_init, start=500)


if __name__ == "__main__":
    #data_tool = GenerateDataFromTrueData()
    # 固定 Python 内置随机数
    random.seed(42)

    # 固定 NumPy 随机数
    np.random.seed(42)

    # 固定 PyTorch CPU 随机数
    torch.manual_seed(42)
    #true_ctlg = read4("odata/ctlg.txt")
    # ======目录 ctlg: (K,3) —— 请替换为你的真实目录 ======
    events, ctlg, proj = read_event_file("data/real.txt")
    # 拟合 GMM 作为先验
    #gmm = fit_gmm_em_torch(
    #    ctlg, n_components=30, n_iters=100, reg_eps=5e-2,
    #    device=device, dtype=dtype
    #)

    # ====== 生成一批数据并演示缺测 ======
    
    rcvs = []
    c = 0 
    event_ids = []
    Tp_obs = []
    Ts_obs = []
    xr = []
    sidxs = [25, 539, 13, 20]    # 只用这一组
    for k in range(4):
        for idx, (eve, ctg) in tqdm.tqdm(enumerate(zip(events, ctlg))):
            #print(eve)
            if idx in sidxs:
                print(idx)
                etime = eve["etime"]
                locinfo = eve["locinfo"]
                rcv = locinfo["rcv"] 
                T_p = locinfo["T_p"]
                T_s = locinfo["T_s"] 
                event_id = torch.ones([len(rcv)], device=device)
                event_ids.append(event_id * c)
                c += 1
                #print("src/rcv/T shapes:", rcv.shape, T_p.shape, T_s.shape)
                t1 = time.perf_counter()
                rcvs.append({"xr":rcv, "Tp":T_p, "Ts":T_s})
                xr.append(rcv)
                Tp_obs.append(T_p)
                Ts_obs.append(T_s)
    event_ids = torch.cat(event_ids) 
    Tp_obs = torch.cat(Tp_obs) 
    Ts_obs = torch.cat(Ts_obs) 
    xr = torch.cat(xr) 
    t1 = time.perf_counter()
    alpha0 = 3.0
    beta0  = (0.5 ** 2) * (alpha0 - 1)  # 假设先验期望 ~ 0.5^2

    out = gibbs_mh_location_multi(
        Tp_obs, Ts_obs, xr,
        event_ids=event_ids, NC=c,
        n_samples=6000, burn=0, thin=1,
        gmm_prior=None, prior_mix=None,  # 或仅用宽高斯
        prop_scale=2.0,
        device_=device, dtype=dtype, verbose=True, 
        alpha0=alpha0, beta0=beta0,
        use_student_t=True,
    )
    t2 = time.perf_counter() 
    print("Gibbs MH 耗时:", t2-t1)
    print("Gibbs MH 耗时(per sample):", (t2-t1)/len(rcvs))
    xs_samples = out["xs_samples"]#.cpu().numpy()
    t0_samples = out["t0_samples"]#.cpu().numpy()
    #ofile = open("odata/reloc.raw.v3.2.txt", "w")
    print(len(events), len(ctlg), xs_samples.shape, t0_samples.shape)
    #41.45272073103115
    #

    xs_samples = xs_samples.float()
    t0_samples = t0_samples.float()
    xs_init = out["xs_init"].float().cpu().numpy()
    single_plot = False          # 拼成 4 幅小图
    plot_sample_path_jgr_sharex_pairs_v2(xs_samples=xs_samples, t0_samples=t0_samples, xs_init=xs_init, start=3000)
    """
    fig = plt.figure(figsize=(12, 6.2), dpi=300)  # JGR 通常较扁的横向排布
    gs = gridspec.GridSpec(2, 4, 
    left=0.08,   # 左边距
    right=0.98,  # 右边距
    top=0.95,    # 上边距
    bottom=0.25, # ⭐ 下面留 25% 高度给 legend
    wspace=0.35  # 子图之间的间距)     # wspace 稍微加大一点
    )
    axs = [plt.subplot(gs[i]) for i in range(4)]
    axs2 = [plt.subplot(gs[i]) for i in range(4, 8)]
    colors = ["#0072B2", "#D55E00", "#009E73", "#CC79A7"]  # Okabe–Ito（色盲友好，印刷也稳）
    burn_color = "0.25"  # 深灰替代纯黑

    start = 500 
    
    for idx in range(4):
        ax = axs[idx]
        ax2 = axs2[idx]
        values = []

        for c in range(4):
            xs_samps, t0_samps = xs_samples[:, idx+c*4, :], t0_samples[:, idx+c*4]
            xs_mean = xs_samps.mean(dim=0)
            xs_std  = xs_samps.std(dim=0, unbiased=True)
            x_mean, y_mean, z_mean = xs_mean.detach().cpu().numpy() 
            xs_samps = xs_samps.cpu().numpy()
            t0_samps = t0_samps.cpu().numpy()
            x1, y1, z1 = x_mean, y_mean, z_mean
            x0, y0, z0 = xs_init[idx+c*4, :]

            values.append(np.stack([xs_samps[start::2, 0],
                                    xs_samps[start::2, 1],
                                    xs_samps[start::2, 2],
                                    t0_samps[start::2]], axis=1)) 
            xs_samps = np.concatenate([np.array([[x0, y0, z0]]), xs_samps], axis=0)
            
            # 只在第一次循环时设置 label
            ax.plot(xs_samps[:start, 0], xs_samps[:start, 1],
                    '-', color="k", lw=0.2, alpha=0.2,
                    label="Burn-in" if c == 0 else None)

            ax.plot(xs_samps[start::2, 0], xs_samps[start::2, 1],
                    '-', color=colors[c], lw=0.2, alpha=0.2,
                    label="Sample Path" if c == 0 else None)

            ax.scatter(xs_samps[start::2, 0], xs_samps[start::2, 1],
                    color=colors[c], s=0.5, alpha=0.2,
                    label="Sampling Points" if c == 0 else None)

            ax.scatter([x1], [y1], marker='+', s=30, c="b",
                    label='Posterior mean' if c == 0 else None)

            ax.scatter([x0], [y0], marker='^', s=30, c="r",
                    label='Start point' if c == 0 else None)

            # ax2 同理
            ax2.plot(xs_samps[:start, 0], xs_samps[:start, 2],
                    '-', color="k", lw=0.2, alpha=0.2,
                    label="Burn-in" if c == 0 else None)            
            ax2.plot(xs_samps[start::2, 0], xs_samps[start::2, 2],
                    '-', color=colors[c], lw=0.2, alpha=0.2,
                    label="Sample Path" if c == 0 else None)

            ax2.scatter(xs_samps[start::2, 0], xs_samps[start::2, 2],
                        color=colors[c], s=0.5, alpha=0.2,
                        label="Sampling Points" if c == 0 else None)

            ax2.scatter([x1], [z1], marker='+', s=30, c="b",
                        label='Posterior mean' if c == 0 else None)

            ax2.scatter([x0], [z0], marker='^', s=30, c="r",
                        label='Start point' if c == 0 else None)

        names = ['X (km)', 'Y (km)', 'Z (km)', 't0 (s)']
        values = np.stack(values, axis=0)
        for k in range(4):
            rhat = gelman_rubin(values[:, :, k])
            print(f"R-hat for component {names[k]}: {rhat:.3f}")
            #ax.invert_yaxis()   # 深度向下为正
            #ax.tick_params(labelsize=7)

    # ===== JGR 风格：面板标注 + 共享 legend =====
    panel_labels = ['(a)', '(b)', '(c)', '(d)']
    for ax, label in zip(axs, panel_labels):
        ax.set_xlabel("X (km)", fontsize=8)
        ax.set_ylabel("Y (km)", fontsize=8)
        # 左上角面板标注
        ax.text(0.02, 1.01, label, transform=ax.transAxes,
                ha="left", va="bottom", fontsize=8, fontweight="bold")
    panel_labels = ['(e)', '(f)', '(g)', '(h)']
    for ax, label in zip(axs2, panel_labels):
        ax.set_xlabel("X (km)", fontsize=8)
        ax.set_ylabel("Depth (km)", fontsize=8)
        # 左上角面板标注
        ax.invert_yaxis()
        ax.text(0.02, 1.01, label, transform=ax.transAxes,
                ha="left", va="bottom", fontsize=8, fontweight="bold")

    # 从第一个子图收集 legend 元素，构造全局 legend
    handles, labels = axs[0].get_legend_handles_labels()

    # 不在各个子图上单独画 legend
    for ax in axs:
        leg = ax.get_legend()
        if leg is not None:
            leg.remove()

    # 全局共享 legend：放在四个子图下面居中
    fig.legend(handles, labels,
            loc='upper center',
            bbox_to_anchor=(0.5, 0.18),
            ncol=5,
            fontsize=8,
            frameon=False)

    # 给 legend 预留一些下边距
    #fig.tight_layout(rect=[0.0, 0.20, 1.0, 1.0])

    fig.savefig("figures/figs/sample_path.png", dpi=300)
    fig.savefig("figures/figs/sample_path.pdf", dpi=300)
    plt.close(fig)
    """
